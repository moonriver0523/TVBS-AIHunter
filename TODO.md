...
> 🔍 **B1／B2 遺漏稽核（2026-07-27）**：B1、B2 跟 B3–B10 不同，**沒有逐條裁定紀錄**，是直接被合成進指南初版的，所以回頭重看了一次。找到 4 處真正掉的，已補進指南：①對仗的內部規則「**動詞對動詞、名詞對名詞**」（第 3 節）②「一則新聞產 **5–10 條標題**、不同切角輪替」（第 7 節）③回扣型收尾的**畫面回扣**（稿內註記 `#回到<開場畫面>`，第 8 節）④快訊 BS 的**內部四段結構**「導言含最新數字→經過重述→官方回應→下一步」（第 0 節語體表）。
>
