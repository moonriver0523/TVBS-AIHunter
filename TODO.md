...
- [ ] **A 步：純統計分析（程式跑，幾乎不耗 token）**——對 `draft_corpus.jsonl` 跑腳本統計：用詞頻率、句長分布、標點/斷句習慣、常見句式與轉場詞，並跟現行字數規則（OS 每行 14 全形字、BAR 16.5–17.5、SOT 標題 17.5–18.5）對照，量出「人味」跟現行機械規則之間的落差。預估 20–40 分鐘、token 數千內（只解讀程式輸出）。
