...
4. **BAR 字卡列表緊接在 `SUPER:` 區塊下方**：依序列出四張 BAR 的編號與完整文字（`BAR 1` / `BAR 2` / `BAR 3` / `BAR 4`），每張約 18–20 個全形字，採電視字卡風格（非文學／書面語）。四張 BAR 的完整文字只在此處出現一次。
5. **內文區的 BAR 只作剪接定位標記**：依序交錯排列 `BAR1` → 其 OS（記者口白，1–2 短段）→ `SB <TC>` → `中文職稱 英文姓名` → 引言內容（「」）→ `BAR2` → OS → `SB <TC>` → 職稱＋姓名 → 引言 → `BAR3` → OS → `SB <TC>` → 職稱＋姓名 → 引言 → `BAR4` → 結尾 OS。內文的定位標記固定寫成無空格的 `BAR1`、`BAR2`、`BAR3`、`BAR4`，**不可在內文重複字卡文字**。最後一個 `BAR4` 之後不再接 SB；全篇須以 OS 收尾，總結後續發展／影響／趨勢，不引入新事實。
