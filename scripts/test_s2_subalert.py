...
# ⚠️ 這裡斷言 `🟡=` 而不是整段措辭：圖例文字後來被 c83717e 統一成「🟡=次重大」，
# 而本測試還卡在舊措辭「🟡=重大未進檔頭」，於是整套測試長期紅一項（2026-08-18 查出）。
# 真正要守的不變式是「有用到才印圖例」，不是那幾個字怎麼寫——措辭綁太死，
# 改文案就假性失敗，久了會把紅燈訓練成雜訊（同 s2_scan.ps1「誤報比漏報更傷」）。
report("用到 🟡 才印圖例", "🟡=" in hdr, hdr)
report("沒用到 🟡 不印圖例",
