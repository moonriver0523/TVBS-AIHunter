...
def mark_for(checkpoint, base_mmdd):
    ...
    固定排程（原則性，使用者可當天隨時特例調整；2026-08-31 起 17:00／20:00／22:00＝晚班，
    16:00／18:00／01:00 已取消）：
    17:00／20:00／22:00／23:00＝晚班（`△`，23:00 是下班前最後一輪）；
    04:30＝無人值守（`▲`）；07:00／08:00＝晨班（`◇`，2026-09-04 起；此前是 `■`、更早是 `●`）；
    ...
    if day >= 1:
        if hhmm < 700:
            return "▲"
        # 🔴 只改生成、不改解析：這裡一律產出 `◇`（2026-09-04 使用者要求，
        # 此前是 `■`、更早是 `●`），但各處正則仍必須認得 `■`／`●`，否則舊檔那些行
        # 會靜默從品質掃與統計裡消失。舊資料不回頭改。
