...
T_PRIORITY = ["烏俄", "美伊", "地緣衝突", "軍事國防", "天災天氣", "政治", "社會",
              "財經", "科技醫藥", "體育", "娛樂藝文", "話題"]
...
def tag_tc(r):
    ...
    # D15（2026-09-07）：非戰爭軍事新聞兜底，只影響沒下 T/C 的則，不求全
    if has(s, "軍演", "閱兵", "軍購", "國防部"):
        T.add("軍事國防")
    ...
    if has(s, "白俄羅斯"):
        C.add("歐洲")
    # D15（2026-09-07）：英國專項獨佔，掛英國就不掛歐洲（同以色列／伊朗）
    if has(s, "英國", "倫敦", "唐寧街", "英相", "白金漢宮", "王室"):
        C.add("英國")
