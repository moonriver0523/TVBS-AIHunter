...
for pre, red, orange, aired, label in (
    ...
    report(f"{label}：is_red/is_orange/is_aired 正確",
           (sv.is_red(line), sv.is_orange(line), sv.is_aired(line)) == (red, orange, aired),
           f"{sv.is_red(line)},{sv.is_orange(line)},{sv.is_aired(line)}")

report("🟠 在代碼之後不算標記", not sv.is_orange("△ AP4676573 (x) ▎內文提到 🟠 符號。"))
