...
CODE = (r"(?:RT\d{4}|APcctv\d{6}|AP\d{7}|[A-Z]{2,6}-\d{1,4}[A-Z]{2}"
        ...
        r"|ABC\d{6,16}"                            # ABC NewsOne（2026-08-10，同樣人工下令才跑）
                                                   # 代碼＝ABC＋Story Number，前 6 碼是 MMDDYY
                                                   # ⚠️ 上限 0810 由 12 放寬到 16：ABC 自家稿是 9 碼
                                                   # （`ABC081026045`），但**加盟台轉供的是 14 碼**
                                                   # （WLS／WISN，如 `ABC08102611055272`）。
                                                   # 12 卡住時症狀正是本檔開頭警告的**靜默漏算**：
                                                   # 那 4 則 txt 印得出來、檔頭則數卻少算 4、品質掃
                                                   # 整行跳過（0810 實錯，靠 render 對帳的 +68 vs
                                                   # 實加 72 這個落差才發現）。
...
# 「隔夜續掃 N 則」只統計 ▲／◇（舊檔的 ■／●）／◆，△ 是基底不計入。
# 一律先剝掉再做其餘比對，否則整行會兩邊都漏辨識（同 SIDE_RE 踩過的坑）。
#
# ⚠️ **晨班（07:00–09:00）的符號換過兩次**：`●` →（2026-08-04）`■` →（2026-09-04）`◇`。
# 兩次都是使用者覺得前一個符號不好認（`●` 與其他圓形標記易混淆；`■` 實心方塊太搶眼）。
# **只改生成、不改解析**：新的 render 一律產出 `◇`，但這裡**必須繼續認得 `■` 與 `●`**——
# 已歸檔的舊 txt（0802／0803… 是 `●`，0804–0904 是 `■`）正則不收的話那些行會 LINE_RE
# 不 match、從品質掃與檔頭統計裡**靜默消失**（數量少算且不報錯，△ 上線時已經踩過一次）。
# 舊資料不回頭改（使用者訂案），所以這兩個相容分支要長期留著，不要當成過渡碼清掉。
LEGACY_MARKS = {"●": "◇", "■": "◇"}     # 舊符號 → 現行符號（同一個時段）
# 順序＝檔頭圖例與「隔夜續掃」的列出順序，照時間排；舊符號 `■`／`●` 緊接在同時段的
# `◇` 之後，這樣舊檔重新渲染時圖例順序與原本一致（不放最後面，否則 ▲／■／◆ 會變成
# ▲／◆／■）。
MARKS = {"△": "23:00 前既有", "▲": "23:00–07:00 新增", "◇": "07:00–09:00 新增",
         "■": "07:00–09:00 新增（舊符號）", "●": "07:00–09:00 新增（舊符號）",
         "◆": "09:00–14:00 新增"}
...
# 0810 實例：`傷亡數字更新` 底下 AP4677621（鐘塔倒塌＋瓦礫堆，畫面好）與
# IN-39MO（**資料靜態畫面**）同組——標在小分題上等於宣稱兩者畫面都好，
# 編輯調出來卻是圖卡。
# 位置：時段標記 → 🔴／🟡 → **🔖** → 代碼，並在第一個備註括號**最前面**
# 寫 `畫面好` 或 `搖晃瞬間`：`▲ 🔴 🔖 AP4677621 (畫面好 …) ▎…`
# ⚠️ 同 🔴／🟤／🟡，**必須一併剝掉**，否則 LINE_RE 不 match、整行會從品質掃與
#    檔頭統計裡靜靜消失（本檔上方那兩條註解記的就是同一種坑，已踩過兩次）。
HILITE_RE = re.compile(r"^(🔖)\s*")
...
def check(path):
    ...
    for n, l, block in side_lines(lines):
        ...
        # ⚠️ SUPER 內容**大體上**不檢查：角色是否「合法」、有沒有 SUPER，一律不命中。
        # 這兩條檢查以前會逼 agent 改角色／無中生有補一個講者，已於 2026-08-02 移除。
        #
        # 🔴 **唯一的例外：`旁白`／`廣告配音`／`群眾`**（2026-08-10 補）。
        # `15-歐印萬掃帶.md` 講者標記那條寫明：新聞段落標成 `旁白` **一律是轉錄誤判**，
        # 這三個角色只准用在廣告、節目預告、片頭片尾、現場群體聲等**非新聞**段落——
        # 而晚班交接裡只會有新聞段落，所以它們出現在這裡**永遠是錯的**。
        # 0810 實例：NHK 熊本地震報導的記者過音有 15 段被標成 `旁白`，
        # 品質掃照樣 0 命中，是使用者自己看出來的。
        #
        # ⛔ **刻意只抓這三個、不做通式角色白名單**——通式正是 2026-08-02 被移除的
        # 那條，它會逼 agent 去動不該動的 SUPER。這裡只點名三個「一定是 BUG」的值，
        # 不問其餘角色對不對，也不要求補上缺漏的 SUPER。
        m_role = re.match(r"^(?:CNN|NHK)\s+(?:\d{2}-\d{2}\s+)?" + _TC
                          + r"\s*（\s*(旁白|廣告配音|群眾)\s*[^）]*）", l)
        if m_role:
            hit(n, f"側錄角色標成「{m_role.group(1)}」——新聞段落不該有這個角色"
                   f"（`15` 講者標記：轉錄誤判，只准用在廣告／預告等非新聞段落）。"
                   f"修正屬上游 `15` 的事，未獲使用者授權前不要自行改角色")
