...
# 時段標記（2026-08-02 訂案，2026-08-03 補 △，2026-08-03 晚再訂邊界，2026-08-04 補 ◆）：素材代碼前帶一個時段符號。
# △＝22:00 前晚班既有（22:00 是晚班最後一輪掃帶，之後留一小時人工最後檢查）、
# ▲＝22:00–05:00 新增（無人值守，含人工檢查那一小時內若有自動掃帶）、
# ●＝05:00–09:00 新增（早班 05:00 上班後才收的素材，讓早班一眼分出上班前後）、
# ◆＝09:00–14:00 新增（日班新增，早班之後、午後晚班備班前）。
# ⚠️ 2026-08-03 起 △ 也要寫出來（原本是「無標記」），三種標記齊全才方便快速瀏覽；
# 「隔夜續掃 N 則」只統計 ▲／●／◆，△ 是基底不計入。
# 一律先剝掉再做其餘比對，否則整行會兩邊都漏辨識（同 SIDE_RE 踩過的坑）。
MARKS = {"△": "22:00 前既有", "▲": "22:00–05:00 新增", "●": "05:00–09:00 新增", "◆": "09:00–14:00 新增"}
        ...
        for mk in sorted(used_marks):
            if mk not in head:
                hit(1, f"素材行用了隔夜標記「{mk}」但檔頭沒有對照圖例"
                       f"（應有 `標記：▲=22:00–05:00 新增　●=05:00–09:00 新增　◆=09:00–14:00 新增`）")
