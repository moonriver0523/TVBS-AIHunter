...
- 🆕 **ABC 每一輪都要掃，沒有跳過表**（`13h` V7-1）——`01:00`／`09:00`／`11:00`／`20:00` 也照掃。
  ABC 沒有 ENEX 那種 31 天實測量，先跟三站同窗跑一段觀察期，
  ⛔ 不要自己憑感覺跳過某一輪，⛔ 也不要拿 0810 那天的樣本外推。
...
4.6. 🆕 **ABC 整併（V7；每一輪都要做）**——照 `13h` V7-3 四步走：
...
- 各站收錄則數、清單對帳結果（窗內未收幾則）——**ENEX 與 ABC 各單獨列一行**
  （ENEX 本輪不掃就寫「本輪不掃 ENEX（V5-1）」；**ABC 每輪都掃，沒有「不掃」這個選項**，
  真的進不去就寫明是靠哪個訊號判定登出）
- 稽核**嚴重／次要**各幾項、怎麼處理
- 本輪標了幾則 🔴（檔頭）／🟡（重大未進檔頭）
- 卡住或被拒絕的地方，**原始訊息照抄**
