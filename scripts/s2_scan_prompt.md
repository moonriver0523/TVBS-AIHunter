...
- **ENEX 只有 01:00／04:30／07:00／17:00／22:00 五輪要掃**（`13g` V5-1；07:30 已併回 07:00；
  2026-09-02 起 20:00 拿掉；2026-09-05 起 01:00 從「跳過」改成「要掃」）。
  本輪 checkpoint 是 `11:00` 或 `20:00` 就**完全不碰 ENEX**，
  回報寫一行「本輪不掃 ENEX（V5-1）」即可，
  且素材會在下一個有掃 ENEX 的輪次自然出現，不是漏收。
- 🆕 **ABC 現在跟 ENEX 同一張表：01:00／04:30／07:00／17:00／22:00 五輪要掃**
  （`13h` V7-1；2026-09-05 使用者裁示：ABC 跟 ENEX 完全同步，不再獨掛 09:00）。
  本輪 checkpoint 是 `11:00`／`20:00` 就**完全不碰 ABC**，
  回報寫一行「本輪不掃 ABC（V7-1）」即可，素材會在下一個有掃的輪次自然出現，不是漏收。
- 🔴 **2026-09-05 起 09:00 這一輪整個從排程刪除**（使用者明確下令，`schtasks` 已移除
  該觸發時間）——不會再有 09:00 這一輪跑，NS/AP/RT 三站也一併不跑，不是「跳過 ENEX/ABC」。
