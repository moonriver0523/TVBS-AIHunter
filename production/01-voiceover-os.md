# 01 — OS 配音（GPT-SoVITS 本人聲音）

> 狀態：**核心已驗證填實**；未知處標 TODO。實測腳本在本機 `D:\voice-training\work\`。

## 目標

把完成文稿裡的**記者 OS（過音）**用本人聲音模型配出來。

## 範圍（哪些要配、哪些不配）

- ✅ 配：記者 OS 過音段（BAR 段的敘述文字）
- ❌ 不配：**稿頭 LEAD**（棚上主播，跳過）
- ❌ 不配：**SB / BITE**（受訪原聲，保留素材原音）

## 模型與環境

- 現用：**`os_voice3` 低 epoch**（去污染乾淨資料 394 段）：SoVITS `SoVITS_weights_v2\os_voice3_e8_s1064.pth`、GPT `GPT_weights_v2\os_voice3-e12.ckpt`
- ⚠️ **選檢查點別挑最高 epoch**：e20/SoVITS12 會「合成感重、很假」（把多支新聞語調平均掉）；降到 GPT e12 + SoVITS e8 音色追平 v1 又保乾淨資料優勢。**乾淨資料 ≠ 好聽，過度訓練反而爛**
- 引擎：GPT-SoVITS v2，conda 環境 `D:\CondaEnvs\GPTSoVits`（torch 2.5.1+cu124）
- 呼叫：`TTS_infer_pack.TTS`，config `tts_infer.yaml` 覆寫 t2s/vits 權重路徑，`device=cuda, is_half=True, version=v2`
- ⚠️ **log 重導向檔案時 stdout 是 cp950**，中文診斷 print 會 `UnicodeEncodeError` 崩 → 啟動前加 `PYTHONIOENCODING=utf-8 PYTHONUTF8=1`

## 參考音（決定輸出乾淨度）

- 從語料自動挑 **SNR 最高**的原始段（腳本 `rank_clean_refs.py`）：SNR = 99pct峰值 / 10pct噪底；篩 `3.2s≤dur≤9.5s`、`speaker_sim≥0.86`
- **不做降噪**（會削高頻變糊）
- prompt_text 取該參考段在 `.list` 的逐字稿

## 合成流程（已驗證）

1. 完成文稿 OS 段 → 切字幕級短句 →（TODO：通用解析器；目前手動分組）
2. **2–3 句一組**合成（甜蜜點；`text_split_method=cut0`，組內用 `，` 串）
   - 逐句更慘、整段跳句，故取中間
   - **尾字修復（已驗證）**：組尾接**犧牲字** `。嗯。`（`gtext = "，".join(lines) + "。嗯。"`）。AR 解碼器收尾常吃掉真正最後一個音節（加 padding 無效，因為根本沒錄到）；犧牲字逼模型把真字念完，再用 ASR 詞級對齊**切在最後一個真字之後 +0.12s**、丟掉犧牲字。**只在 ASR 覆蓋足夠真字數才切，否則保留全長**（不誤切）。實測 weak 8→3、尾字補回
3. **int16 修正 + 峰值正規化**（防爆音，硬約束）：
   ```python
   y = np.asarray(audio, np.float32)
   if np.max(np.abs(y)) > 2.0: y = y/32768.0
   # 全組串接後：
   block = block/(np.max(np.abs(block)) or 1.0)*0.8
   ```
3. **開頭雜訊裁切（2026-07-28 新增，已驗證）**：組首偶爾會多吐 1–2 秒**非語音的雜訊**。合成＋ASR 後，把**第一個被辨識到的字之前**的音訊整段切掉（`head = max(0, words[0].start - 0.12)`），並把該組所有詞的時間戳同步平移 `-head`。這與既有的尾字裁切是同一招用在另一端。**覆蓋率驗收抓不到這種錯**——它只問「字有沒有唸到」，不問「有沒有多唸出字以外的東西」（見 `09` A7）
4. **驗收**：每組 ASR（faster-whisper large-v3）
   - 覆蓋率 = 逐句拼音在 ASR 結果的 in-order 命中率；`cn2an` 正規化數字（避免 32≠三十二 誤判）
   - 門檻 `0.80`；未達 → 換 seed 重試
   - seed 池：`[42,1042,2042,7,123,888,3407,9999]`
   - **語言偵測**：ASR 偵測非中文 → 該組廢棄（非中文＝一定不是本人）
   - **雜訊偵測（2026-07-28 新增）**：除了覆蓋率，另外掃「**有聲音但沒有任何辨識詞覆蓋**」的區段（詞前後各給 0.15 秒寬限，長度 ≥0.6 秒且 RMS >0.02 才算）。有殘留就視同弱句換 seed；挑 best take 時以 `(覆蓋率, -雜訊總長)` 排序，不要只看覆蓋率
   - ⚠️ **繁簡多音字會讓覆蓋率誤判**：`還`(huan)／`还`(hai)、`著`(zhu)／`着`(zhe) 預設讀音不同，ASR 出簡體、稿子是繁體時同一句會算成沒唸到。比對前先 `opencc t2s`，並額外把 `著` 換成 `着`（opencc 不動它，因為簡體也有這個字）
5. 全掛句（8 seed 都 <0.80）→ **編輯刪除**（多為修辭句/與 SB 重複句，可割捨；腳本 `trim_os_lines.py` 依 cue 邊界切）
   - **同一格位連續失敗就換句法，不要只換 seed**（2026-07-28 `胰癌新藥1600` 實測）：某一句被整句吞掉時，先試「換分組（2 行 vs 3 行）」與「改寫句首」，句首是 `就先…` 這類虛詞起手最容易被吞。四種寫法都掛就刪掉該行、把事實併進鄰句
6. 輸出：乾淨 OS wav（每段一檔）+ 逐句時間戳 JSON（給字幕 `03` 用）
7. **終檢**：逐句 cue 區間 RMS ≥0.02（防「有字幕沒配音」）

## 硬約束（不可違反）

- 輸出前驗 `clip% = mean(|y|>0.99) == 0`（見 `09` A1）
- 不逐句合成、不降噪參考音
- **每組音訊的頭尾都必須貼著實際語音**：頭部裁到第一個辨識詞之前，尾部裁在最後一個真字之後（見 `09` A5／A7）

## 待補（TODO）

- [ ] 完成文稿 → OS 短句 + 分組的通用解析器（目前手動）
- [ ] 合成/驗收腳本（`synth_os_groups_v3.py`）從 `D:\voice-training\work\` 整理進 repo `scripts/`

## 模型改進路線（TODO，依 CP 值排序）

現況 `os_voice3` 低 epoch 為「堪用」，非此路線天花板。使用者 2026-07-25 認定仍有改進空間。

- [ ] **1. 參考音改成分段挑選**（成本最低、預期最有感）
  - 現在全片固定用 `os_0169` 一段當 ref；ref 決定語氣、語速、音色細節，影響大於直覺
  - 做法：語料 ref 池先分類（平述／問句／加重語氣／快慢），每段 OS 挑語氣最接近的 ref
  - 零訓練成本，可單獨 A/B 驗證
- [ ] **2. 補語料要補「句型種類」不是「數量」**（約一個下午）
  - 394 段偏少且全是同一種新聞平述句；模型沒聽過問句、數字、長句、加重語氣 → 遇到就露餡（使用者聽到的「有點不清楚」多半是這個）
  - 再錄 20-30 分鐘涵蓋上述句型，效益大於再錄 100 段同型稿
- [ ] **3. 升級模型世代**（成本最高、上限最高）
  - 現用 GPT-SoVITS **v2**；v3/v4 在音色相似度與**尾字截斷**上都有改善（現在是靠 A5 犧牲字硬解，v3+ 較不需要）
  - 同級替代可評估：IndexTTS2、CosyVoice2、F5-TTS
  - 需重建環境 + 重訓，留到要當常態工具再做
- [ ] **4. 原始錄音品質**（若前三項到頂仍不滿意）
  - 來源若是過音成品而非乾淨母帶，房間殘響與壓縮會被模型一起學走，**後製救不回**，只能重錄
