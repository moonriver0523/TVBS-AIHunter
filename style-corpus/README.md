...
- **閱讀狀態標籤**：`tag_unread.py` → `reading_status.json`——全部 1701 篇的 `B1`/`B2`/`unread` 標記。未讀 1601 篇（<1200 字 367、1200–9000 字 1183、9000–20000 字 49、>20000 字 2）。**日後若需針對性補抽（特定題材/語域），從 `unread` 池取樣即可，不會重讀。**（注意：`reading_status.json` 尚未計入 B3 的 60 篇，實際未讀約 1541 篇；重跑 `tag_unread.py` 前需先讓它支援 `sample_B3/`。）
...
- ⚠️ **語料污染警訊（B3 發現）**：2026 年語料已混入 AI 輔助產出稿（`0622-2` 留有【主播稿頭】【總長度】等產稿模板痕跡）。日後補抽與人味萃取時，含產稿模板痕跡的稿件應降權或排除。
