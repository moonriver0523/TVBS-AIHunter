...
**開始前必讀**：[`../common/07-bite-assistant.md`](../common/07-bite-assistant.md)（找 TC 與 SB 格式）、[`../common/08-execution-efficiency.md`](../common/08-execution-efficiency.md)（瀏覽器操作與下載重試）。本文件只寫本流程特有的規則。
...
- **TC 第一站是 Video Transcript 面板**（有逐行時間碼就停）。使用者已給定引言時，**不要**為了剪一句把完整 shotlist／VIEW MORE 全文貼進對話。
- **Restrictions 一句話回報**即可；無限制就說無。
- **步驟 3 只輸出該 TC 區間**的雙語逐字，不附前後大段 transcript。
