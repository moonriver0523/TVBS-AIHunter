...
5. ⚠️ **RT Next 換頁後的文字擷取殘留（2026-08-02 實測兩次重現）**：在 RT 詳情頁按 Next（›）後，URL 與畫面都已是新項目，但 `get_page_text` 可能回傳**上一則的完整內容**——不是失敗、是**無聲抓錯**，直接信任會把 A 代碼寫進 B 內容。**防範鐵律**：Next 換頁後拿到的文字，**必須先核對文中 Edit No（或標題）與當前 URL 的編號一致**才能用；不一致 → 等 1 秒重讀一次；再不一致 → 改用截圖讀取，並照第 2 條記錄。首次點卡片進詳情的當次讀取沒有此問題。
