...
- ✅ **NS 地方小品（DONUT／小市場 VO／人情趣味）一律收錄，不准以「地方性」自行排除（2026-08-05 使用者訂案）**：0805 NS 補掃時，agent 曾以「極地方性小品新聞」為由自行跳過 MW／SE／NE 系列共數十則（如德州動物園消暑、田納西理髮師免費理髮、愛達荷 Tartar 野火一家人徒手滅火守住家園等），使用者要求全部補收。**這類素材本身沒有機械判準可濾**（不像 §後面的 `AUDIO TRACK`／`GRAPHIC` 佔位公告那樣有 `footageType` 可辨識），agent 不得憑「新聞價值主觀判斷」自行降階排除，一律照三段式格式收錄。真正可以不收的只有：①同一 story 的重複 take（多支 Edit No 描述完全相同事件，只收 1 支，其餘視為重複跳過）②`footageType` 為 `AUDIO TRACK`／`GRAPHIC`＋初稿佔位（見下方機械排除）③空白樣板或純數字跑馬燈（`script` 裡 `--SOT--`／`--VO SCRIPT--` 等欄位全空，或內容只是股市指數數字，無敘事文字）④同一支長專訪被切成多段 `ISO`／`CLEAN AIR` 重疊分段時，只收 1 支濃縮版即可，不必逐段全收。
...
**一道指令跑完七項離線檢查，數秒完成、零瀏覽器呼叫、零 API。**
...
**需要開瀏覽器，所以獨立做**——但它是**價值最高的一項**（0805 靠它抓到 47 則漏收）。做法：重撈各站清單 → 跟狀態檔 diff → 算出窗內未收的。

⚠️ Playwright 只有一個 persistent profile，**開之前先確認掃帶輪次沒在跑**，否則互鎖（0803 空窗八小時）。
⚠️ RT 清單是內層容器虛擬捲動、首屏只有約 10 則——做法見 §1b 第 3 條。

📌 **`src_text` 與 `_rt_list_{HHMM}.json` 落檔規則存在的理由就是這個模組**：有它們，⑥ 之外的檢查全部能離線做；沒有的話每次稽核都得重開瀏覽器，成本差一個數量級。
