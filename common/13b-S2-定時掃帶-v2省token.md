...
⛔ **固定輪就是這三站，不多不少。** ENEX（`members.enex.news`）與 ABC NewsOne（AdBridge）
雖然掃帶／摘要都已打通，但**使用者訂案「人工下令才跑、不進固定排程」**（2026-08-09／2026-08-10）。
規則在 [`18-交換平台素材整併.md`](18-交換平台素材整併.md)，⚠️ **本輪不要自己跑去掃**——
一輪的時間預算是照三站抓的，多掃一站會排擠掉窗內零漏收。
📌 那兩站的素材由人工輪次寫進 `_待整併/`，本輪照 §4b 當一般候選檔整併進來即可，**跟你要不要去掃是兩件事**。

✅ **NS 白名單：掃帶全程只會用到這兩個網址，其餘一律不對**（2026-08-04 訂，取代原本的禁用路徑黑名單——黑名單列不完，0804 同一個錯犯了兩次）

| 用途 | 網址 | 何時用 |
|---|---|---|
| 保活／取 token | `https://newsource.ns.cnn.com/landing` | 每輪開工第一件事；兩輪之間的 58 分鐘續命 |
| 清單＋全文 | `POST https://newsource-content-api-530.ns.cnn.com/api/v3/stories` | 掃清單、拿稿、pending 重查——**全部走這支** |

⛔ **除了這兩個，不該出現任何其他 NS 網址。** 尤其：

- **`/search`（或 `/search?q=…`）→ 回 Page Not Found。** NS **不支援用網址直接搜尋**（2026-07-21 已確認，原記於 [`cnn/01`](../cnn/01-auto-script-writing.md)）。⚠️ **0804 同一個錯犯了兩次**——會犯是因為「我要查某一則」時很自然就想拼一個搜尋網址，但**掃帶從頭到尾都不需要搜尋**：要查特定素材就用上面那支 API 撈回來自己比對 id，不要開 UI。
- **`newsource.cnn.com`（缺 `.ns.`）→** 導向「隱私權設定發生錯誤」頁，`get_page_text`／截圖都讀不到。

🔍 **NS 頁面不對時的懷疑順序（先自己再站方）**：①**自己的網址不在白名單上**②Playwright profile 被鎖（見 §1a-0）③登入態過期。
⚠️ **不要一看到頁面不對就開調查「站方卡點」**——0804 兩次都是這個形狀，站方其實好好的。核對網址只要一秒，開調查要花掉大半輪。
...
- 保活的實際做法（兩輪之間怎麼補一次）見 §5 防卡設計第 3 條。
...
> 📌 **今天是第一輪（狀態檔還不存在）→ 先看 [§5a 每天第一輪的額外動作](#5a-每天第一輪建檔輪的額外動作2026-08-04-補)**，那是建檔輪專屬的七項清單（`window_start`、機動格、前一天歸檔…）。漏掉不會報錯，只會讓整晚的檔頭數字默默不對。第二輪之後照 §5 的每輪流程即可。
>
...
- 🔁 **「舊稿重上」（refile）照收不誤，但第一個 `()` 備註必須標 `{MMDD}舊稿`（2026-08-05 使用者訂案）**：RT 常把數日前建立的稿子在今天 refile 一個新版次重新推上清單（`RESENDING WITH COMPLETE SCRIPT`／`REFILE` 等字樣），**這在 RT 是常態、不是異常**。判準看 **guid 內嵌的日期碼**：`newsml_RW{4碼EditNo}{DDMMYYYY}RP1:{版次}` 中間那 8 碼就是**原始建立日期**，若它早於今天，這則就是舊稿重上。
  - **處置：照常收錄**，內容照三段式正常寫，**不因為是舊稿就跳過**——當天 refile 上來就代表 RT 認為它現在仍有新聞價值。
  - **標記方式**：在**第一個 `({備註})` 括號內**寫上 `{MMDD}舊稿`，`MMDD` 取 **guid 的原始建立日期**（4 碼，如 7/31 建立寫 `0731舊稿`），與其他備註以空格分隔。例：`RT2286 (0731舊稿 烏克蘭Fire Point飛彈防禦整合歐洲技術) (BITE) ▎…`
  - **為什麼要標**：編輯看到代碼與當天檢查點時，會預期這是當天發生的事；沒有這個標記就無法一眼判斷素材的事件時間點，可能誤把數日前的內容當即時新聞用。
  - ⚠️ **這與 §Edit No 跨日重複是兩回事**：跨日重複是「同一個 Edit No 對到不同新聞」（查證時的抓錯風險），舊稿重上是「同一則新聞用新版次重新上架」（收錄時的標記義務）。查證仍以 headline 比對為準。
  - 📌 **0805 實例**：`RT2286`（Fire Point 飛彈防禦）7/31 建立、8/4 16:35 refile 為 v5，當時漏收；0805 補收時依本規則標為 `(0731舊稿 …)`。漏收根因與 §RT 清單首屏假設有關，見 `needs-review` 的 `RT2286-missed-root-cause`。
...
- 🎫 **NS 保活（keep-alive）：每 58 分鐘續一次門票（2026-08-04 使用者訂案）**
  ...
  - 🔴 **保活一律用 `mcp__browser__*`（Playwright），絕對不准用 `mcp__claude-in-chrome__*`（2026-08-05 實錯訂正）**：0804 晚班排 01:58／02:56／03:54 三次保活 cron 時，prompt 沒寫死工具，執行時誤用了 claude-in-chrome——那是**另一個瀏覽器 profile**，跟掃帶用的 Playwright persistent profile 完全無關。三次保活全部誤保到不相干的 profile，實際負責掃帶的 Playwright NS 登入態在 01:00 後沒人碰、於 1 小時滑動時效內悄悄過期，直到 04:30 開工才發現整段被踢回登入頁，中間空窗近 3.5 小時、事後得靠使用者人工重登才救回。**排 NS 保活 cron／交辦保活任務時，prompt 必須明白寫「用 Playwright（`mcp__browser__*`）」，不能只寫「開瀏覽器」這種模糊字眼**，執行時也要先確認自己叫的是哪一組工具再動作。
...
- ✅ **NS 地方小品（DONUT／小市場 VO／人情趣味）一律收錄，不准以「地方性」自行排除（2026-08-05 使用者訂案）**：0805 NS 補掃時，agent 曾以「極地方性小品新聞」為由自行跳過 MW／SE／NE 系列共數十則（如德州動物園消暑、田納西理髮師免費理髮、愛達荷 Tartar 野火一家人徒手滅火守住家園等），使用者要求全部補收。**這類素材本身沒有機械判準可濾**（不像 §後面的 `AUDIO TRACK`／`GRAPHIC` 佔位公告那樣有 `footageType` 可辨識），agent 不得憑「新聞價值主觀判斷」自行降階排除，一律照三段式格式收錄。真正可以不收的只有：①**逐字完全相同的清單重複列出**（標題與 `script` 內容**逐字一致**，純粹是清單重複列同一支——只有這種才算重複，見下方紅框）②`footageType` 為 `AUDIO TRACK`／`GRAPHIC`＋初稿佔位（見下方機械排除）③空白樣板或純數字跑馬燈（`script` 裡 `--SOT--`／`--VO SCRIPT--` 等欄位全空，或內容只是股市指數數字，無敘事文字）。

🔴 **同一事件的多段／多版素材一律各自收錄，不准併（2026-08-06 使用者訂案，推翻原 ①④）**

原本的 ①「同一 story 的重複 take 只收 1 支」與 ④「多段長專訪只收 1 支濃縮版」**已刪除**——那兩款是 0806 連三輪 NS 漏收的直接來源，而且**三個不同輪次的 agent 都是照規則做的**（`0700round-dedup-note`／`NA-107WE-dup-excluded`／`1049round-dedup-note` 三則備註都用了規則原詞「重複 take」）。⚠️ **這不是 agent 誤解，是規則本身把「同一事件」和「同一支素材」混為一談。**

| 情形 | 舊規則 | **現行** |
|---|---|---|
| 多段專訪 PT1／PT2／第 1–4 段 | 只收 1 支濃縮版 | **每段都收**，備註標 `(PT2)` 等 |
| `BUTTED SOTS` 乾淨訪問剪輯版 vs 主稿 | 視為重複 | **都收**——BUTTED SOTS 本來就是給編輯挑乾淨 BITE 用的獨立素材，不是主稿附屬品 |
| 同事件 VO／SIL 版 vs `PKG` 完整版 | 只收 1 支 | **都收** |
| 同事件不同 SOT 剪輯 | 併入一支 | **都收** |
| 標題＋`script` **逐字完全相同** | 只收 1 支 | 只收 1 支（**唯一保留的重複情形**） |

- **判準＝機械的**：`bitcentralId` 不同就是不同素材。**只有逐字比對相同才准跳過**，不准用「同一事件」「內容近似」「較不完整」「重複監視器畫面」這類語意判斷。
- ⚠️ **「不確定就收」**——不要臆測合併。少收一則是真漏收，多收一則只是多一行。
- 📌 跨站真重複（同一事件被 NS＋AP＋RT 各報一次）是**另一回事**，那屬整併階段的判斷，不適用本條。
...
- 🎯 **BITE 判定＝看 `footageType`（機械判準；2026-08-04 依實測全表訂正）**：三站統一「有無 BITE 由機器判、BITE 內容才是 LLM 的事」。⚠️ NS 稿件**不用 `SOUNDBITE` 這個詞**（0804 實測 60 則全部 0 個），引言段標記是 `--SOT--`——**不要拿 AP／RT 的關鍵字習慣來掃 NS 稿件**，會全部誤判成無BITE。
  ...
  **完整對照表**（0731–0804 共 15 種 `footageType`、實際庫存交叉統計，非憑印象列舉）：
  ...
  🎯 **`batch.json` 的 NS 項目必帶 `footage_type`**（2026-08-04 起，與 RT／AP 的 `sb_count` 同一機制）：抽取白名單已經有 `ft` 欄位，原樣帶上即可。`add-batch` 會把「必有 BITE 類卻寫無BITE」的單筆**照收、並把疑慮寫進 `needs_review`**，PKG 灰區只印提醒。不帶＝不檢查（向下相容），但**新輪次一律要帶**。

  🔴 **兜底只判斷「BITE 標記對不對」，不判斷「該不該收錄」（2026-08-05 訂正，原本是拒收閘門）**

  原本 `sb_count` 與 `footageType` 兩條判準都寫成**拒收**（整筆丟棄不入庫），那是設計錯誤——**把統計上的「零例外」當成了邏輯上的必然**。0805 實錯 `SE-005WE`：曼菲斯晨間新聞主播直播中打瞌睡的花絮，`footageType=RAW` 但**真的沒有任何引言**（稿內只有 `(pause :12 seconds for nat)`留 12 秒自然音）。**`RAW` 是「原始素材」，不是「一定有人講話」。**

  更糟的是**失敗模式無聲**：被擋的素材不入庫、不建 `needs-review` 殼，只在終端機印一行，對話結束就消失——使用者與下一輪 agent 都無從得知曾經有過這則、也分不清是被機器擋掉還是 agent 判斷不收。

  **現在的行為**：一律**照收**，疑慮寫進該則的 `needs_review`（`resume` 每輪會列出「待人工」，確認後用 `needs-review done --ids …` 結案；若是漏寫、補上 BITE 後 `update-entry` 會**自動結案**）。
  ⛔ **不要再改回拒收**——兜底的價值是提醒漏標，不是替使用者決定收不收。
  ⚠️ **`add`／`update-entry` 與 `add-batch` 行為一致**（0805 補）：原本單筆完全不檢查，是意外留下的後門——那則被擋的素材就是靠 `add` 繞過收進來的。**兩條路徑不一致時，人會往阻力小的那條走，兜底等於形同虛設。**
...
3. 🔴 **清單是「內層容器的虛擬捲動」，必須捲那個容器才看得到全部（2026-08-05 實測訂正，這條害過一次大漏收）**
   ...
   **實測一次可拿約 60 則、涵蓋前 5 個多小時**，正常輪次的窗完全夠用，**不需要碰 Load More**。
   ...
   🔴 **但有兩種情況一定不夠，必須按 Load More 補撈**（2026-08-09 轉正式模式時補；
   測試模式有「每站上限 5 則」擋著，這個缺口一直被掩蓋，正式模式立刻會咬人）：
   - **窗比較長的輪次**——`13:00→16:00` 建檔輪、`04:30`／`07:00` 這種隔了 2.5–3 小時的，
     素材量大時 60 則可能蓋不到窗的起點。
   - **補掃失守窗**——前面有輪次沒跑到（斷電、登入態掉了），窗會被拉長好幾小時。

   **判準很機械，不用猜**：撈到的清單裡**最舊那一則的上站時間，如果還晚於本輪窗的起點**，
   就代表窗的前半段根本沒進視野 → **要按 Load More 繼續撈，直到最舊一則早於窗起點為止**。
   ⚠️ 這跟 `13` 的「LOAD MORE 禁則」不衝突：那條禁的是**拿 Load More 當主掃法**
   （會把最新的窗擠掉），這裡是**先撈完本頁、確認不夠才往前補**，方向相反。
   📌 0809-0430 實例：清單只有 12 則、最舊 `RT4117` 是 01:57，而本輪窗從 01:00 起算——
   當時因為是測試模式已收滿 5 則就沒補撈，正式模式**這種情況就是真漏收**。
...
4. **Load More 三條行為規則**（實測，僅在捲到底仍不夠時才用）：⑴ 是**換頁替換**不是累加——按下後最新那批從 DOM 消失，所以**必須先撈完目前這頁才准按**，按完再撈一次做聯集去重；⑵ 只認**真實點擊**，合成 JS click 無效；⑶ ⚠️ 原本寫「深夜窗通常首屏 10 則就夠」是**錯的**（0805 推翻）——首屏 10 則是虛擬捲動只渲染這麼多，不代表窗內只有 10 則。
   ...
   🔴 **⑵「合成 JS click 無效」不是提醒，是死鎖警告**（0827-1000 實錯：`page.evaluate` 裡自己寫
   `element.click()`＋`setTimeout` 遞迴重試「按到 8 次才 resolve」，因為 `.click()` 點了等於沒點、
   DOM 永遠不變，迴圈又**沒有找不到按鈕/沒有進度時的退出條件**，卡了快 30 分鐘、整輪拖到 65 分才收工）。
   - **正確做法**：用 Playwright MCP 的 `mcp__browser__browser_click`（帶真實 CDP 滑鼠事件，等同真人點擊）
     點 Load More，⛔ **不要**在 `browser_evaluate` 裡用 `element.click()`／`dispatchEvent(new MouseEvent(...))`
     這類合成點擊去按它——這條規則沒有例外，不是「先試試看」。
   - **重試迴圈一律要帶上限**：不管用什麼方式點擊，任何「按了再撈、撈不到就重按」的迴圈都要有
     **明確的 stall/timeout 出場條件**（例如「連續 N 次點擊後清單則數沒增加就放棄，改記 `needs_review`」），
     ⛔ **不准寫成「達到 N 次成功點擊才 resolve」這種只認成功、不認失敗的迴圈**——那正是 0827-1000
     卡死的形狀，點擊失敗時永遠也數不到 N 次成功、Promise 永遠不會 resolve。

5. ✅ **收工前必做的自我檢查（0805 新增）**：把清單撈到、且**上站時間落在本輪窗內**的 Edit No 全部列出來，跟狀態檔 diff。**只要有一則窗內的沒進庫，就不算掃完** —— 不論原因是抓不到詳情、判定跳過還是任何其他理由，都要 `needs-review add` 記下代碼與原因。⛔ **不可以只回報「已找到的都收了」**：那句話在視野被縮小時永遠為真，卻掩蓋真正的漏收。

   💾 **diff 用的清單一定要存進當晚暫存資料夾**（`{YYYYMMDD}/_rt_list_{HHMM}.json`，含 `code`／上站時間／`guid`）。**沒存等於這輪的視野沒有留下證據**——0805 下午兩輪都沒存，事後要查漏收只能重開瀏覽器重撈一次清單，而站方清單會滾動、更早的素材已經捲出範圍就永遠查不回來。存了之後任何人都能離線用一支 python 對帳，零瀏覽器呼叫。⚠️ 這條與「有沒有真的做 diff」是兩件事：0805 19:00 那輪 diff 做對了（窗內零漏收）**但沒存檔**，所以我仍然得重撈一次才驗得出來。舊的 LOAD MORE 禁則是為 Next 鏈設的，本流程不走 Next 鏈故不適用；退回舊流程時禁則照舊。
...
- 🎯 **RT／AP 每筆必帶 `sb_count`（2026-08-04 起，BITE 機械兜底）**：抽取白名單已回傳 `sb_count`（RT）／`sb_count`＋`has_sot`（AP），寫 batch.json 時**原樣帶上**（`{"id":…,"sb_count":4,…}`）。`add-batch` 會擋「`sb_count > 0` 卻寫『無BITE』」的單筆（跳過並要求把引言寫進 `▎BITE：` 段重送）；`update-entry` 用 `--sb-count N` 帶入，同樣會擋。SNTV 列表級沒有全文，**不帶** `sb_count`（不帶＝不檢查，向下相容）；⚠️ **但 §4 兩層判準裡
「開了詳情」的那一格必須帶**（2026-08-09），既然開了就有這個數字，不帶等於自己製造盲區。**NS 改帶 `footage_type`**（2026-08-04 起）——NS 稿件不用 `SOUNDBITE` 這個詞、數不出 `sb_count`，兜底改看 `footageType`：必有 BITE 的七種直接擋、`PKG` 只印提醒，對照表見 §1a-3。
- 💾 **每筆必帶 `src_text`＝瘦身後的原文，落檔留存（2026-08-04 訂案）**：`batch.json` 每筆再加一個 `src_text`，放**抽取白名單瘦身後的稿件全文**（RT 的 `story`／AP 的 `script`／NS 的 `script`，已經去過 tag 與 metadata 的那份），`add-batch` 會忽略這個未知欄位、不影響入庫，但檔案本身留在 `{YYYYMMDD}/` 暫存資料夾裡可事後查。
  - ⛔ **`src_text` 裡只准有站方原文，不准寫任何自己的判斷、說明或備註**（2026-08-09 訂，實錯）。
    要記判斷就用 `needs-review add`，那才是它的位置。
    **為什麼這條特別重要**：`src_text` 是**事後離線查證的唯一依據**，混進中文說明之後，
    每一個讀它的機制都會不可靠——**RT4131 實錯**：agent 在原文尾巴接了一段
    「（說明：sb_count=0 係因路透 captioned 格式…無 SHOTLIST 段…數不到 SOUNDBITE 字樣…）」，
    那段話裡的 `SHOTLIST`／`SOUNDBITE` 字樣**害假 BITE 判準誤判、同一則誤報連續四輪**，
    每一輪都要人再查證、再寫一次備註。
    📌 稽核③ 現在會**指名列出**混入說明的素材代碼（機械判準：三站原文一律英／西文，
    出現中文就是 agent 寫的）；分析時也會自動把尾端的中文段剝掉，但**那是防呆不是授權**。
...
- 🔴 **收工前一定要跑 `s2_mark_ingested.py --apply` 幫本輪吃過的 `_待整併/` 交件檔改名**
  （2026-08-16 補：這一步本來就寫在 [`13`](13-S2-定時掃帶.md)「`_待整併/`」節，但沒有任何腳本自動呼叫它，
  完全靠掃帶輪 agent 記得手動跑——0816 那輪就漏了，改用臨場自己取的名字
  `已整併 {原檔名}` 手動改掉，不符合腳本認的 `已入庫_{原檔名}` 前綴，
  導致這支腳本自己都認不出那個檔已經處理過）：
  ```
  python scripts/s2_mark_ingested.py --file "…/{MMDD}-s2-state.json"          # 先看報告，確認全部入庫
  python scripts/s2_mark_ingested.py --file "…/{MMDD}-s2-state.json" --apply  # 沒有缺漏才改名
  ```
  ⛔ **不要手動改檔名**——`--apply` 用固定前綴 `已入庫_`，人工另取名字（哪怕語意相近，
  例如「已整併 」）下一個 agent／腳本都認不出來，形同沒改。
- 🔴 **分類收斂：由獨立 agent 出建議，掃帶輪只負責套用（2026-08-11 使用者訂案，
  覆蓋 08-10 那版「23:00 掃帶輪自己做」）**

  **為什麼要拆出去**：每一輪都會分類**自己收的那批**，但**跨輪的一致性沒有人負責**。
  0810 實例（哥倫比亞強震）：卡利的素材散在【強震速報】三個小分題、佩雷拉被歸進
  【卡利災情】、馬尼薩雷斯被歸進【波哥大災情】——每一輪各自看自己那批，沒有人回頭看
  整天長成什麼樣。缺的不是「分類」，是**收斂**。而 23:00 已經是全天最重的一輪
  （0810 跑了 25.6 分，還要清 pending、跑最終稽核、render 定版），再塞一件需要
  全局判斷的事進去，最先被犧牲的就是它。

  | | 分類收斂 agent | 掃帶輪 |
  |---|---|---|
  | 何時 | **輪次之間**（狀態檔沒有檔案鎖，同時寫就是 lost update） | 固定輪 |
  | 做什麼 | 讀 `list-topics --subs`／`s2_topic_review.py`／`s2_topic_dedupe.py`，**只看名稱層**，寫出建議檔 | 跑一支指令套用 |
  | 寫狀態檔 | ⛔ **不寫** | ✅ 寫 |
  | 成本 | 約 5K token | 近乎 0（機械套用） |

  - 交辦範本：[`scripts/s2_reclass_prompt.md`](../scripts/s2_reclass_prompt.md)（只指路，不重複規則）
  - 建議檔：`_待整併/{MMDD}-分類收斂建議.txt`，只認兩種指令行——
    `MOVE {id}={大分類}/{中主題}/{小分題}`、`ORDER {大分類}={中主題1};{中主題2};…`
  - **掃帶輪套用**（收工前、render 之前）：

    ```
    python scripts/s2_apply_reclass.py --file "…/{MMDD}-s2-state.json" \
        --suggest "…/_待整併/{MMDD}-分類收斂建議.txt" --dry-run
    # 看過沒問題再拿掉 --dry-run，然後照常 s2_render.py
    ```

    建議檔不存在就**跳過，不要自己生一份**。套用階段⛔**不再做判斷**——判斷已經在
    建議檔裡了，掃帶輪重做一次只是多一次打錯的機會（素材 id 含空格，如
    `CNN 08-10 224714`，手拼分號字串特別容易斷錯）。
    id 不存在時該支會**整份拒絕套用**並列出來，不會只套一半。

  ⛔ **不要逐則重讀內文重分類**——那要 4–5 萬 token，是這一步的 8～10 倍，而且逐輪
  已經開過小分題了，收斂只補「跨輪才看得出來」的一致性問題。
...
- 🔴 **品質檢查在「寫入當下」就會跑，不是只在 render（2026-08-11 起）**：
  `add`／`add-batch`／`update-entry` 寫完會立刻印

  ```
  ⚠️ 格式待修 N 項（**已入庫**，請直接用 `update-entry` 改掉，不要等 render 才發現）
  ```

  看到就當場修，**不要留到收工**——那時已經忘了脈絡，還要重讀一次素材。
  ⚠️ 只警告不擋下（擋下會中斷整輪，代價比格式瑕疵大），所以**它不會替你修**。

  > **為什麼加這道**：品質掃原本只綁在 `s2_render.py` 上，等於「有人記得 render
  > 才會被檢查」。0811-0100 那輪 01:20 收工並 render 完畢後，**另有 35 則素材在
  > 01:33–01:35 才寫進狀態檔**——那批完全沒經過任何一次品質掃，交接單就帶著 52 項
  > 格式問題掛在那裡給編輯看。**輪次外寫入等於從側門進來，直接繞過關卡。**
  > 把關卡挪到寫入當下，不管誰寫、有沒有 render 都躲不掉。
  > 📌 側錄、YouTube 兩行式與空內容不套這套判準（那是另一套格式），不會誤報。
...
## 2a-2. 每輪快速稽核：`s2_audit.py`（2026-08-06 上線，**交班前必跑**）

```
python scripts/s2_audit.py --mmdd 0805
```

**一道指令跑完七項離線檢查（數秒、零瀏覽器呼叫），加上第八項清單對帳（要一份剛撈的清單快照，見下）。**

| # | 檢查 | 抓什麼 |
|---|---|---|
| ① | `checkpoint` 格式 | 沒有 4 位數字群 → 時段標記會塌成 `△`（0806 實錯 85 則） |
| ② | 時段標記分佈 | 跨夜輪次全部是 `△` ＝ 幾乎確定有問題 |
| ③ | BITE 一致性 | 假 BITE（`(BITE)` 但 `sb_count=0`）、漏標、`src_text`／`sb_count`／`footage_type` 有沒有帶 |
| ④ | 中主題 | 同格內名稱互相包含、跨格同名、**放錯大分類** |
| ⑤ | 暫存檔落地 | `_rt_list_{HHMM}.json` 有沒有留 |
| ⑥ | 結構化欄位 | `fields`／`parse_ok`／`pending`／`needs-review` 未結案 |
| ⑦ | 品質掃 | 呼叫 `s2_validate check` |

**離開碼**：0＝全過；1＝有 🔴。🟡 是提醒、不影響離開碼。

### 誰跑、什麼時候跑

- **當輪 agent**：整併完、`render` 之後、交班前跑一次。🔴 直接處理掉再交班。
- **另一個 agent（複核）**：**這是本模組的主要設計目的**——0805–0806 那一夜，RT 漏收 36＋11 則、NS 漏 5 則、時段標記錯 85 則、AP 假 BITE 7 則、中主題 92→62，**每一項都是事後才發現，當輪 agent 全部回報「正常完成」**。共同形狀是**當輪 agent 看不見自己的盲區**：它按規則做了、也覺得做完了，但規則有洞或視野被縮小。**只有換一雙眼睛才看得到。**
- ⛔ **🔴 的項目要直接處理，或用 `needs-review add` 記錄後再交班——不要只在回報裡講一句**。0805 的 NS 被擋就是「說了但沒寫進狀態檔」，結果狀態檔零痕跡，是事後重撈才發現的。
...
**多半做得到**——稽核的時機通常是掃帶剛結束、Playwright 空著。⚠️ **但不保證**：0806 10:00 輪就因為 profile 被別的 process 佔著（cron 監控／另一個工作 agent）而沒做成清單對帳。**搶不到就記 `needs-review` 說明沒做、下次補**，不要硬等也不要略過不提。0805 靠這一項抓到 **47 則漏收**（RT 36＋11），是所有檢查裡回收最高的。
...
⚠️ **開瀏覽器前先確認下一輪掃帶還沒開始**，否則 Playwright 互鎖（0803 空窗八小時）。

🔴 **對帳做完會自動留痕，render 收工時會查（2026-08-06 上線）**

跑過 `--rt-list` 等參數之後，結果會寫進狀態檔的 `_top.reconcile_log[{checkpoint}]`；
`s2_render.py` 收工時去查這一輪有沒有紀錄，**缺哪一站就指名喊出來**：

```
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
⚠️  這一輪（0806-1300）沒有做清單對帳：AP／NS 缺
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
```

**為什麼要做這個**：0806 RT 兩次漏收都倒在「這一步沒被執行完」——07:00／08:00 是
還不知道有這工具，10:41 是**知道、寫下「留待下次補做」、然後直接 render 收工**，
就再也沒回去。**缺的從來不是能力，是「一定會做」的保證。**

- ⚠️ **只警告不擋**：無人值守時段硬擋會讓整輪卡死（違反 §5「半夜禁問」）。
- **真的做不了就 `needs-review add` 寫明原因**，不要無聲跳過。
- 📌 **已裁定不收的不會重複報**：只要該素材代碼出現在任何 `needs-review` 備註裡、
  且備註含「不收／重複／排除／未收／跳過」字樣，往後就列在「已裁定不收」不再算漏收。
  ⚠️ **重複誤報會把警告訓練成雜訊，那比不報還糟**——要讓它閉嘴就得留痕，正是要的紀律。

📌 **`src_text` 與 `_rt_list_{HHMM}.json` 落檔規則存在的理由就是這個模組**：有它們，①–⑦ 全部能離線做；沒有的話每次稽核都得重開瀏覽器，成本差一個數量級。
...
## 2d. 檔頭時間窗：`window_start`（2026-08-04 訂正）

檔頭第 2 行的時間窗是「**這份交接檔開檔至今累計掃了多久**」，不是最後一輪的區間。

- **建檔那一輪設一次就好**：`python scripts/s2_state.py set-top window_start "2026-08-04 13:00"`
  （值＝第一輪掃描窗的**起點**；16:00 那輪掃的是 13:00–16:00，就寫 `13:00`）。**之後每輪都不要再動它。**
- 終點由 render 自己取頂層 `checkpoint`，agent 不用管。
- ⚠️ **不要再靠 `--window` 傳**：漏傳或格式不合（連字號前後沒空格）檔頭就默默壞掉，而且不報錯。
  `--window` 現在只當特例覆寫用，正常輪次不需要。
- ⚠️ **`window_local` 不是這個**：它存的是**單輪**掃描區間、每輪覆蓋，拿來當檔頭會讓五小時的交接檔
  看起來只掃了兩小時（0804 實錯）。它仍可保留作單輪紀錄，但 render 不再優先採用。
- 沒設 `window_start` 時 render 退回「最早的 checkpoint」，**會少算第一輪往前涵蓋的那段**
  （16:00 那輪的 13:00–16:00 就看不到），所以第一輪務必設好。
...
## 3b. 中／小分題檢視：`s2_topic_review.py`（**render 前必做**，2026-08-09 上線）

🔴 **每一輪 render 之前都要跑一次、而且要看過整張表**（使用者 2026-08-09 要求）：

```
python scripts/s2_topic_review.py --file "<狀態檔>"
```

**為什麼 §3a 不夠**：`s2_topic_dedupe` 只做「同名小分題跨大分類」那一種高信度訊號，
**設計上就不管同一大分類內的同義不同名**。0809 實測它回報「0 命中」，但交接檔裡同時存在：

| 形態 | 0809 實例 |
|---|---|
| 同義不同名 | 體育【高爾夫】＋【高球】 |
| 同事件拆成兩個中主題 | 社會【泰國校園槍擊】＋【校園槍案】（小分題還寫「泰國校園」） |
| 同事件拆成多個小分題 | 足球底下「悼念老梅西／梅西家族／梅西父喪／**美斯家事**」——四個都是同一件事 |

⚠️ **不要只看它列出的候選清單**，那只是省眼力用的：
- **機械只認字面**——港譯「美斯」對台譯「梅西」**零共同字，永遠挑不出來**，那種只有人看得出來。
- **候選會有誤報**（例如「足球 vs 美式足球」被算成 100% 相似）。**自己判斷，不要照單全收。**
- 判準已經收緊過一次：第一版噴出 53 組、大半是「美國野火 vs 美國社會」這種同組通用字造成的雜訊。
  現在會先扣掉「同組裡出現 4 次以上」的字（`美國`／`球`），降到 20 組。
  📌 **會這樣調是因為誤報的代價**：重複誤報會把警告訓練成雜訊，那比不報還糟。

合併方式：`s2_state.py set-category --pairs`，改完再 render。
⛔ **只印不改**——分類的自動修正在「全盤檢討分類問題」結案前一律不做（見 TODO）；
本支只碰中／小分題的同義合併，**不碰大分類歸屬**，兩者是不同的軸。
...
- 🔴 **訂正（2026-08-09）：`role` 含 SOT 是「必要條件」，不是「充分條件」——不可據此直接標 `(BITE)`**
  上面那條與 §1a-2「`sb_count=0` 不得標 `(BITE)`、不得硬湊」**會打架**：列表級不開詳情就沒有
  `sb_count`，照上面做就可能產出假 BITE，而 §1a-2 正是為了防這個而立的（0805 AP 實錯 7 則）。
  **兩條都沒錯，錯在把必要條件當成充分條件**——根因 §276 早就寫過：`editorialrole` 含 `SOT`
  只代表**素材形式含現場聲**，不代表**有可引用的引言**。
  **實例 AP5467093**（WTT 桌球橫濱）：`role=VOSOT`，但整份 SHOTLIST 只有比賽逐分、
  一句 SOUNDBITE 都沒有——當輪靠多開一次詳情才沒有產出假 BITE。

  **改用兩層判準**（成本只加在真正分不出來的那一小撮）：

  | 列表級看到的 | 怎麼標 | 要不要開詳情 |
  |---|---|---|
  | `cap` 裡有引號引言（`"…"`） | 標 `(BITE)`，引言**照抄** `cap` 原文 | ❌ 不用（零額外呼叫） |
  | `role` 含 SOT 但 `cap` 沒有引言 | **開一次詳情**取 `sb_count`：>0 標 `(BITE)`、=0 標 `無BITE。` | ✅ 只有這一格要開 |
  | `role` 是 `VO` 等無聲形式 | 標 `無BITE。` | ❌ 不用 |

  ⛔ **刪除原本的「寫不出引言原文就寫講者＋內容概述」**——那句正是假 BITE 的產生器：
  沒有引言原文就代表你不知道那個人講了什麼，「內容概述」是從標題猜的。
  **寧可標無BITE 讓編輯自己去看帶子，也不要給一個調出來只有環境音的 `(BITE)`。**
  📌 開詳情那一格記得**照 §533 帶上 `sb_count`**，否則稽核③ 又變成盲區。
...
> ⚠️ **路徑順序（2026-08-03 更新）：§1a API 直查（首選）→ §1b 清單直開（第一層退路）→ 本節 UI 舊做法（最後退路）。** 本節的 RT Next 鏈（8b）與 AP modal 相關條目**只在 §1b 也失敗時才使用**。其餘防卡條目（resume、跳站、半夜禁問等）不分路徑一律適用。
...
2. ⚠️ **`checkpoint` 一律用 `{MMDD}-{HHMM}` 格式，不要只寫時間（2026-08-06 實錯）**
   `mark_for()` 是靠 checkpoint 裡的 **4 位數字群**判時段的（`0806-0130` → 認得出日期與時分）。
   **只寫 `01:30`／`04:30` 會抓不到 4 位數字群**，判不出時間就退回預設 `△`——
   0806 凌晨兩輪共 **85 則無人值守時段（應為 `▲`）的素材被標成 `△`**，
   早班翻開會誤以為是晚班就有的。⚠️ **失敗是無聲的**：不報錯、只是標記錯。
   補掃輪加後綴沒問題（`0806-0430-RT補漏` 照樣認得），**但日期不能省**。

3. 🎫 **兩輪之間顧好 NS 保活（58 分鐘門票）**：NS 登入態是 1 小時滑動時效、過期只能人工重登（機制與證據見 §1a-3「NS 登入態」框）。**掃帶 agent 自己在等待期做**——距上次接觸 NS 接近 58 分鐘就 `browser_navigate` 開一次 `newsource.ns.cnn.com/landing`、確認 `isAuthenticated`，然後照常收工關瀏覽器。
   🔴 **2026-08-07 起改由獨立排程做，agent 不必再管**（`scripts/s2_keepalive.ps1`）

   **改用工作排程器之後，「agent 在等待期做」這條就失效了**——排程模式下 agent 跑完
   就退出，**根本沒有在等待的 agent**。這是搬去排程時漏掉的破口，0807 才發現：
   當時 NS 已經登出（上次接觸是前一天 13:00），若沒發現，當晚 22:00 那輪 NS 會整站掃不到。

   **現行做法**：排程 `S2-NS保活` 每 30 分鐘（`:20`／`:50`）跑一次，載入 `/landing`
   把 JWT 續期。**用 Playwright 直接開，不叫 Claude**——這件事只是開一個頁面，
   零 token、約 10 秒；一天 48 次若走 `claude -p` 成本會很可觀。

   - **鎖的優先序：掃帶 > 保活**。兩者共用 `.s2-scan.lock`。撞到時**保活直接放棄**
     （掃帶第一站就是 NS，它自己會續期）。⚠️ **反過來絕對不行**——保活只握鎖 10 秒，
     但若讓它排隊等，就可能卡住一輪 20 分鐘的掃帶。**寧可漏一次保活，不可漏一輪掃帶。**
   - **NS 登出時**（離開碼 3）寫進 `D:\Downloads\S2掃帶log\_NS保活.log`，
     標記 `*** NS 已登出，需要人工重新登入 ***`。**過期就沒有任何後備憑證**，只能人工。
   - ⚠️ **腳本必須用 MCP 同一個 `--user-data-dir` 與同一個 chromium 版本**：
     profile 不同讀不到登入態（0807 實錯：多加 `channel:'chrome'` 變成用 Google Chrome
     開同一個資料夾，症狀長得像「沒載入 cookie」）；chromium 版本不符則是 launch
     後 context 立刻關閉。

   ⏱️ **（以下為 0807 之前的舊做法，手動掃帶時仍適用）基準點＝這一輪的開工時間**（2026-08-04）：掃描順序固定 **NS → AP → RT**（見 §0），NS 是每輪第一站，所以「上次接觸 NS」就等於整點開工那一刻，續命時刻直接用整點推算即可。⚠️ 若哪次破例把 NS 排到後面，這個推算就不成立——要改用**實際打開 NS 的時間**當基準，不能照整點算。⛔ **不可另開獨立 agent／排程做這件事**，Playwright 只有一個 persistent profile，會跟掃帶互鎖（0803 空窗八小時的坑）。下一輪馬上要開工就不必多跑（開工本身就會續期）。
4. ⛔ **不准 `taskkill`／`Stop-Process` 殺 Playwright 或 node 進程（2026-08-06 實錯）**
...
5. **RT 卡住跳站**：連續 2 次 Next 沒反應／頁面沒變 → 記下目前 Edit No.（`needs-review add`），跳去掃 AP／CNN，回報 RT 中斷點。不准死磕（V1 已知 SPA 卡快取雷）。
6. **半夜禁問**：無人值守時段遇到需使用者確認的事項（可疑素材、分類拿不準、腳本壞掉）→ `needs-review add` 記錄＋寫進交接檔備註，**繼續往下跑**。不得 `AskUserQuestion` 等回應、不得停住。⚠️ **NS 登入過期是此原則的典型適用場景**：無人值守時段發現 NS 卡登入頁，記進 `needs-review` 並跳過 NS，不要停住等使用者。
7. **fallback 全部單向**：階梯只往下走（選擇器→整頁→截圖），不回頭重試上一步。
8. ⚠️ **RT 連掃＝單分頁 Next 鏈＋「換頁後必須刷新再讀」（2026-08-02 三輪實測定案，取代先前雙分頁法）**
...
8b. ⚠️ **Next（›）灰掉按不動時（2026-08-02 實測，使用者指出主因）**：
   ...
   - 只有窗內素材已捲出首屏時才非用 Next 鏈不可；此時 Next 仍失效 → 照第 5 條記錄中斷點並跳站。
   - 只有在「窗內素材已捲出首屏」時才需要 Next 鏈；此時若 Next 仍失效，照第 5 條記錄中斷點並跳站。
9. **AP `/home` 偶發跳轉到其他頁（2026-08-02 實測，已見兩種變體）**：navigate 到 AP Newsroom `/home` 或點側欄 Latest 後，偶爾會被 SPA 路由帶去別的頁面——已見過 `/live`（Live Feeds）與 `/home/foryou`（訂閱推薦頁），根因同樣是路由不穩，**不是特定跳去哪一頁的問題**，之後遇到別的變體（例如 `/archive`）也比照處理。**每次讀列表前先確認 URL 是 `/home`**；不是就重新點側欄 Latest（點文字正中央），最多重試 2 次，仍失敗照第 5 條跳下一站並記錄。

## 5c. 排程啟動器：`scripts/s2_scan.ps1`（2026-08-06 上線）

排程不叫 `CronCreate`（三個結構性限制，見 `TODO.md`），改由 **Windows 工作排程器**叫這支：
...
| # | 做什麼 | 為什麼 |
|---|---|---|
| ① | **互斥鎖** | Playwright 單一 persistent profile，兩輪並行必互鎖（0803 空窗八小時、0806 撞兩次）。用**作業系統層獨佔檔案握把**——程序當機／被砍，握把由 OS 回收，**不會留下解不開的死鎖**（用「檔案存在與否」當鎖就會）。已實測四種情境 |
| ② | **撞到就跳過，不排隊** | 上一輪還在跑就寫一行進 `_跳過紀錄.txt` 然後離開，**離開碼 0**（排程器不該當成失敗）。漏的事後補掃，比兩輪打架好 |
| ③ | **checkpoint 由它算** | `{MMDD}-{HHMM}`，不寫死在 prompt 裡（0806 教訓：指令裡寫死的值三小時後就過期） |

- **prompt 範本**：`scripts/s2_scan_prompt.md`，`{CHECKPOINT}` 會被代換。
  ⚠️ **範本只放本輪參數、不複製規則內容**——規則天天在改，複製一份就會過期並與本檔衝突。
- **驗產物不只看離開碼**：跑完會檢查 `{MMDD}-s2-state.json` 與 `{MMDD}晚班交接.txt` 在不在，
  缺了就記進 `_跳過紀錄.txt`。**離開碼 0 不等於掃帶做完了**（0806 就是這樣驗出 AP 整站 0 則）。
- **排程設定完先 `-DryRun` 跑一次**：只印會送出的 prompt，不叫 `claude`。
- ⛔ **這支腳本自己也不殺任何進程**（同 §5 第 4 條）。profile 被佔就是別人正在用。

## 5a. 每天第一輪（建檔輪）的額外動作（2026-08-04 補）

> 🔴 **2026-08-10 起，第 1 項與建檔本身已改由腳本做掉，不再是你的工作。**
...
> FileNotFoundError，**退回昨天那份繼續寫**，再改名輸出——昨天的 398 則整包
> 被當成今天的，而且**全程沒有一步報錯**。根因是 `s2_state.py default_file()`
> 回傳「最新修改的那份」，建檔輪那一刻最新的必然是昨天。
> 📌 **這種事不該靠任何人記得**，所以移到腳本層。
> 現在 `default_file()` 遇到「已過 16:00 卻沒有今天的檔」會**直接報錯**，不再靜靜用昨天那份。
>
> **你要做的只剩**：開工時確認 `{MMDD}-s2-state.json` 是今天的日期、且 `items` 是空的。
> 不是就**停下來回報**，⛔ 不要自己建、不要退回用昨天那份。

第一輪跟後續輪不一樣：狀態檔還不存在，有幾件事**只在這一輪做一次、做完就不要再動**。漏掉不會報錯，只會讓整晚的檔頭數字或歸檔位置默默不對。

| # | 動作 | 指令／說明 |
|---|---|---|
| 1 | **確認前一天的檔已歸檔** | `自動掃帶系統/` 這層應該只剩今天的。昨天的 `{MMDD}-s2-state.json`／`{MMDD}晚班交接.txt`／`.prev.txt`／**`{MMDD}晚班交接.html`**（2026-08-09 新增，別漏）若還在，搬進 `Archive/{YYYYMMDD}/`（用昨天的日期）。<br>🔴 **`.html` 一定要一起搬**：編輯用手機看的 Apps Script 網址是抓「最後修改時間最新」的那份 html，舊的留在正式層，萬一今天的 html 因故沒產出，網址就會**無聲退回昨天的資料**。⚠️ **只搬昨天的，不要碰今天的**；搬完不要重新渲染舊檔<br>🔴 **動手歸檔前先確認「昨天的」真的是昨天的**（2026-08-09 實錯）：晚班交接檔是**跨夜**的——`0808` 那份要一路用到 **0809 13:00**，在那之前它是**今天還在用的檔**，不是昨天的。判準：那份的 `checkpoint` 若是今天的日期（如 `0809-0800`），**它就還在服役，不准歸檔**。 |
| 2 | **`resume`** | ⛔ **「共 0 則」不是證明，它同時也是最常見的失敗形狀**（2026-08-09 實錯訂正，原文寫「這就是確認自己真的在建新檔」，等於替錯誤背書）。<br>**0809-0900 實錯**：agent 跑 `resume` 沒帶 `--file`，讀到不存在的預設路徑 → 顯示「共 0 則」→ 依原文判定自己在建新檔 → **把還在用的 `0808-s2-state.json` 整個歸檔、另開新檔**，資料斷成兩份（沒丟，人工併回）。<br>**改用兩個旁證，缺一不可**：<br>①**現在是不是 16:00 那一輪**？建檔輪只有 16:00（見 `13` 時刻表）。**其餘 11 輪一律是續掃**，看到 0 則就是**讀錯檔**，停下來查 `--file`。<br>②`自動掃帶系統/` 這層**還有沒有 `{MMDD}-s2-state.json`**？有就是它還在用，**不准歸檔、不准另開**。<br>📌 `--file` 沒給時腳本現在會自動挑資料夾裡最新的那份（2026-08-09 修），但**仍然要明寫**——自動挑是防呆，不是授權你不寫。 |
| 3 | **`scratch-dir --mmdd {晚班起始日MMDD}`** | 建立當晚暫存資料夾，本輪起所有中間檔寫進去 |
| 4 | ⭐ **`set-top window_start "{YYYY-MM-DD HH:MM}"`** | 🔴 **值＝前一天最後一輪的排定時刻，也就是 `13:00`**（2026-08-09 使用者訂正）。新的一天第一輪是 **16:00**，所以收錄窗口就是 **13:00→16:00** 接起來，**常態下一律是前一天 13:00**。<br>⚠️ **不要照「前一檔實際跑到哪一輪」去算**——那在漏跑時會算錯：0808 因斷電，10:00／12:00／13:00 三輪都沒跑，前一檔最後的 checkpoint 停在 `0808-0800`，照字面算會得到 08:00（少報 5 小時、且把沒人掃過的空窗算進涵蓋範圍）。**要的是排程設計上的交界點，不是實際跑到哪裡。**<br>📌 那段沒跑到的窗屬於「失守窗」，用 `needs-review` 記，**不是靠 window_start 去表達**。<br><br>（以下為原始說明，仍適用於非常態情況）值＝**前一檔最後一輪的 checkpoint 時間**——去 `Archive/` 看昨天那份 state 的 `checkpoint`（例如 `0806-1212`）就寫 `2026-08-06 12:12`，**從那裡接續才不會留空窗**。設了之後**每輪都不要再改**，檔頭時間窗才會是「開檔至今累計」。詳見 §2d<br>⚠️ **這個值一律自己去前一檔算，不要照開工指令裡寫死的數字**（2026-08-06 實例：指令寫的 `08:00` 是三小時前判的，期間又跑了 `1130`／`1212` 兩輪，早就過期；當輪 agent 自己改成 `12:12` 是對的）。前一檔找不到時才退回「本輪掃描窗起點」 |
| 5 | **機動格（第一格大分類）** | ⛔ **由使用者當天決定，agent 不自行開、不自行命名**。使用者有指定才 `set-top special_category "{名稱}"`；沒指定就空著，不要沿用昨天的 |
| 6 | **pending 清查** | **跳過**——新檔沒有 pending，不必空跑 |
| 7 | **跨日主題檢查** | **不用手動做**：`s2_render.py` 跑完會自動叫 `s2_topic_dedupe`，它自己會去讀前一天的檔當種子（見 §3a）。前一天的檔找不到時只會印一行提示，不是錯誤 |

- **狀態檔不用先建**：第一次 `add-batch` 會自動建立，不存在也不會報錯。
- 第 4、5 兩項都是 `set-top`，可以連著跑完再開始掃。
- ⚠️ **這七項只有第一輪做**。後續輪次照 §5「防卡設計」的每輪開工流程即可（`resume` → `scratch-dir` → 掃 → pending 清查 → 整併 → render）。
