...
- **時段標記由 render 依 `first_seen_checkpoint` 自動補**（2026-08-03 晚訂正邊界）：22:00 前＝`△`、22:00–05:00＝`▲`、05:00–09:00＝`●`（22:00 那輪算 `▲`）。理由與細節見 `13`「隔夜續掃」節——晚班 22:00 最後一輪掃帶、留一小時人工檢查後 23:00 才正式收班，早班改 05:00 上班。**補掃輪**（例如 09:10 撈回稍早漏掉的素材）checkpoint 判不準，用 `s2_state.py set-mark` 逐則寫死。
