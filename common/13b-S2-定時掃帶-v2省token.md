...
(韓聯社 記者連線 03:24) 韓聯社記者於首爾現場連線，說明…（200字摘要）
```
...
| **形式** | **三選一**，判斷準則見下 |
| **摘要** | **約 200 字**（比通訊社素材長，因為沒有 `畫面：`／`BITE：` 分段）。事件、關鍵數字、誰說了什麼一次講完 |
| **不要的東西** | ❌ 不寫 `▎畫面：`／`▎BITE：`／`▎無BITE。`（那是通訊社單行式的欄位）；❌ 不加 `(BITE)` 第二括號 |
