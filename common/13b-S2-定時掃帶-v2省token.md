...
| # | 動作 | 指令／說明 |
|---|---|---|
| 1 | **確認前一天的檔已歸檔** | `自動掃帶系統/` 這層應該只剩今天的。昨天的 `{MMDD}-s2-state.json`／`{MMDD}晚班交接.txt`／`.prev.txt` 若還在，搬進 `Archive/{YYYYMMDD}/`（用昨天的日期）。⚠️ **只搬昨天的，不要碰今天的**；搬完不要重新渲染舊檔 |
| 2 | **`resume`** | 新的一天會顯示「共0則」——這就是**確認自己真的在建新檔**、沒有誤用到昨天檔案的檢查點。若顯示有幾百則，停下來確認 `--file` 的日期對不對 |
