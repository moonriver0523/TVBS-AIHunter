...
**實測記錄**：2026-08-03 共跑 **6 輪**——AP 6/6＋8/8＋10/10＋12/12、RT 6/6＋4/4＋5/5；最終版全部 **殘留實體 0、殘留 tag 0**。第 5–6 輪另做**交叉核對**：AP 的 API 前 8 則與頁面 DOM 前 8 則**編號與順序逐位完全相同**；RT 的 guid 推導 Edit No 與 DOM 清單吻合（DOM 是虛擬化清單只渲染視窗內 ~8 則，API 抓到而 DOM 沒渲染的屬正常，**不是抽取錯誤**）；early-access 項目（稿未到）`story` 為空、`early:true` 正確標記。
