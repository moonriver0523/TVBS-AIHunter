...
| # | 動作 | 指令／說明 |
|---|---|---|
| 1 | **確認前一天的檔已歸檔** | `自動掃帶系統/` 這層應該只剩今天的。昨天的 `{MMDD}-s2-state.json`／`{MMDD}晚班交接.txt`／`.prev.txt` 若還在，搬進 `Archive/{YYYYMMDD}/`（用昨天的日期）。⚠️ **只搬昨天的，不要碰今天的**；搬完不要重新渲染舊檔<br>🔴 **動手歸檔前先確認「昨天的」真的是昨天的**（2026-08-09 實錯）：晚班交接檔是**跨夜**的——`0808` 那份要一路用到 **0809 13:00**，在那之前它是**今天還在用的檔**，不是昨天的。判準：那份的 `checkpoint` 若是今天的日期（如 `0809-0800`），**它就還在服役，不准歸檔**。 |
