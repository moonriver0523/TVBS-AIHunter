...
| 大分類／T/C | `13f`「各產線 T／C 由誰下」／「來源預設 C」兩表 | ⛔ **不可自己叫 `add-batch`**（見本檔 V7-3）：前兩步由 `s2_platform_merge --apply --in-round` 內部做，跑完再自己下 `set-tc`。⚠️ T（議題）沒有預設、一律照內容判、⛔ 不要自創；ABC **沒有預設 C**（`13f` 明訂 ABC／ENEX 不得自創預設） |
