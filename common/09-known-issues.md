...
**解法：** 不要對整個資料夾做 `ls -la` 或 `grep -r`。**先用 `find -newermt` 按時間窄縮範圍**（例如只抓最近 1~2 天修改的檔案），再對縮小後的結果 `grep` 關鍵字；需要確認特定檔名是否存在時，優先用 `Glob` 工具而非 Bash `ls`（`Glob` 不受檔名特殊字元影響 shell 解析）（2026-07-24「破一百1200」案例，找 210219/210329 對應側錄檔時踩到）。
