# S2 定時掃帶 V3（精簡版）——測試中，尚未上線

> **狀態：測試版（2026-08-11 由 13b V2 精簡而成，尚未上線）。**
...
>
> **上線方式**：把 `scripts/s2_scan_prompt.md` 規則清單第 2 項從 `13b` 換成本檔。
> **Rollback**：改回 `13b` 即可，`13b` 原檔未動、永久保留當歷史檔案（含全部實錯脈絡與驗證紀錄）。
> **日後改規則**：上線後改本檔；要查「為什麼有這條」回 `13b` 或 git 歷史。
