...
### ⭐ 檢查點：**只有 5 輪要掃 ABC**（ENEX 的四輪 ＋ 09:00）

🗳 **2026-09-05 使用者裁示**：ABC 比照 ENEX 的四輪，另加 `09:00`。

| 檢查點 | 掃 ENEX？ | 掃 ABC？ | 該輪實際站數 |
|---|---|---|---|
| **01:00** | ⬜ 跳過 | ⬜ **跳過** | NS → AP → RT |
| 04:30 | ✅ | ✅ | NS → AP → RT → ENEX → ABC |
| 07:00 | ✅ | ✅ | NS → AP → RT → ENEX → ABC |
| **09:00** | ⬜ 跳過 | ✅ **掃**（使用者加的那一輪） | NS → AP → RT → **ABC** |
...
⚠️ **這張表跟 ENEX 那張的來歷不一樣，要知道差別**：ENEX 的四輪是 **31 天實測量**訂的；
ABC **沒有那個數據**（手上只有 0810 一天週日偏斜樣本、0831 的 3 天人工量 18／13／5 則），
這張表是**使用者裁示**。計畫書 §4-2 原本建議「先全掃一段觀察期再定子集」，
使用者選擇直接定表，⚠️ 所以 **V7-6 的觀察期回寫項仍然有效**：
上線後累積各 checkpoint 實際量，發現 09:00 沒東西、或被跳過的輪次其實有量，
...
- **抓不到 `Categories` ⛔ 不等於「沒有 Sports」**——是**判準失效**。
  這時要①改用 `Script` 裡的訊號補判（見下），②並在回報裡寫明「本輪 Categories 抓不到」。
