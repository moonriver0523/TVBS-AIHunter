...
ENEX 有跳過表是因為有 **31 天實測量**（`13g` V5-1）。**ABC 沒有**——手上只有 0810 一天
（週日，形狀偏斜）與 0831 抓到的 3 天人工量（18／13／5 則）。計畫書 §4-2 明訂：

> 先讓 ABC 用「跟三站／ENEX 同一個窗」跑一段觀察期（7～14 天），累積各 checkpoint 實際量，
> 觀察期滿再比照 `13g` V5-1 的方法定子集。

所以 **V7 第一版：每一個檢查點都掃 ABC**，包含 `01:00`／`09:00`／`11:00`／`20:00`。
⛔ **不要自己憑感覺跳過某一輪**，也⛔ **不要拿 0810 那天的樣本外推平日**。
...
- **`DeliveryAvailableDateTime` 直接就是台北時間**（站方讀 `tzOffset` cookie 換算，
  掃帶 profile 一直是台北），⛔ 不要再換算一次。
- 回傳的**陣列長度＝窗內真實則數**，直接餵 V7-5 的清單對帳。
- **`Length`（`05:00`）與 `Categories` 都是現成欄位**——ABC 這兩格比 ENEX 好，不必再量。
...
1. **拿 `detailId`**：CSV 模式**沒有**這個欄位。把同一支 `NewsSearch` **不加 `getCSV`** 再打一次取 HTML，
   從中撈 `/Delivery/Detail/{id}` 的連結，用 `Story Number` 對映回 CSV 那筆。
   ⚠️ **這條在 repo 裡沒有逐字驗證過的選擇器**（0810 只記「從清單 HTML 撈 Detail ID」）——
...
**只看它會誤判成「已經續到了」**。判準用這兩個具體訊號：

- `document.querySelector('input[name=__RequestVerificationToken]')` **不見了**；或
- `NewsSearch` 回的**不是 CSV**（`content-type` 不是 `text/csv`，或內容是登入頁 HTML）。
...
python scripts/s2_platform_extract.py check-entries --entries "<scratch>/abc_entries_{HHMM}.json"

# ① 機械封裝成候選檔
python scripts/s2_platform_extract.py abc \
...
# ② 交件前 lint（有 ❌ 就停下來修）
python scripts/s2_platform_lint.py "<scratch>/{MMDD}-ABC-state.json"

# ③ 整併進狀態檔（⛔ 一定要 --in-round，否則護欄 100% 誤擋，理由見 13g V5-3）
python scripts/s2_platform_merge.py "<scratch>/{MMDD}-ABC-state.json" \
...
- **時長不必自己寫**：`extract` 會拿 CSV 的 `Length` 自動補成行尾 `▎MM:SS`（2026-09-04 起）；
  你自己已經寫了就以你的為準、這支不覆蓋。
...
- `_token_metrics.jsonl` 的分桶**不認 `enex`／`abc`**，會落進「其他」桶。遙測歸因會糊掉，
  不影響掃帶本身。
- ABC **沒有 checkpoint 跳過表**（V7-1），所以每輪都多一站的成本；觀察期滿再定子集。
...
- 觀察期（7～14 天）滿後，比照 `13g` V5-1 的方法定 ABC 的 checkpoint 子集，回寫 V7-1。

---

<!-- RULES-EOF 13h 2026-09-04 — V7 五站增量（ABC 進固定輪，尚未上線） -->
