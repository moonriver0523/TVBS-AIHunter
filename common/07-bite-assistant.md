...
**先確認方向，再跑全片轉錄（重要，避免白工）：** 既有雙語逐字稿通常已有區塊級TC，先從中提出1-2句候選（可用 `AskUserQuestion` 或直接列出候選）並取得使用者方向確認後，才對整支側錄檔案（可能長達數分鐘到10分鐘以上）跑完整 `video_analyze`（`filters: {transcription: true}`）取得逐秒精確TC；不要在使用者尚未確認要哪一句／哪個方向前，就先跑全片轉錄。
