...
**BITE 講者不限受訪者，且要主動找，不是被動允許**：記者／主播的口白（現場連線、棚內播報）只要有可核對的逐字來源（既有逐字稿、官方字幕、或依上述優先序取得的TC），一樣可以當作 BITE 使用，不是只有受訪者引言才算——見下方方式2範例的「白宮記者 Kevin Liptak」。**起稿前盤點BITE候選時，不能只挑官方文稿裡標成 SAYING／SOUNDBITE／SOT 的受訪者引言段落**：若某筆素材是記者現場連線或主播稿頭、沒有被特別標記的引言區塊，仍要主動讀一遍內容，判斷裡面有沒有數據、關鍵論點、預測等適合直接引用當BITE的句子，不要因為沒有明顯標記就直接歸類成「無BITE、只能拿來寫OS」。2026-07-20「通膨升息1730」案例中，前面幾次盤點都只顧著找受訪者引言，直到使用者明確提醒才回頭從兩段記者現場連線裡挑出BITE。
...
**先確認方向，再跑全片轉錄（重要，避免白工）：** 既有雙語逐字稿通常已有區塊級TC，先從中提出1-2句候選（可用 `AskUserQuestion` 或直接列出候選）並取得使用者方向確認後，才對整支側錄檔案（可能長達數分鐘到10分鐘以上）跑完整 `video_analyze`（`filters: {transcription: true}`）取得逐秒精確TC；不要在使用者尚未確認要哪一句/哪個方向前，就先跑全片轉錄——2026-07-20「追殺川普1730」案例中曾先跑完10分鐘側錄檔的全片轉錄，結果使用者對候選方向答「放棄」，該次轉錄等於白工。
