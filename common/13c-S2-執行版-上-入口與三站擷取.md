# 13c　S2 定時掃帶 V9（執行版）— 上：入口與三站擷取

> **V9（2026-09-14）**：以 V8 為底，只改「三站正常批次路徑」一段（消除舊「選用／手寫 batch」與 `from-raw` 骨架互斥）。V8 原檔備份於 `common/v9/_v8bak/`，`scripts/s2_v9_switch.ps1 -Off` 一鍵切回。
...
（2026-09-07 訂正：ENEX 自 V5、ABC 自 V7 皆進固定輪，見本檔 V5-1／V7-1；不再是人工下令才跑）
...
| ABC | `ss-tok` cookie（HttpOnly） | **約 1 小時**，滑動續期（本檔 V7-1） |
...
| **11:00** | ✅ **（2026-09-10 起改掛）** | 使用者裁決改回要掃，取代原本「不掛 ENEX」的裁示 |
...
ABC **排最後**，理由與本檔 V5-1 把 ENEX 排最後同一套：讓「三站跑完」的時間點盡量早，
NS 的 1 小時 JWT 才不會在輪次尾端的空窗期悄悄過期。

- **本輪不掃 ENEX 時**（本檔 V5-1 的跳過表），順序就是 **NS → AP → RT → ABC**，
...
| **11:00** | ✅ **（2026-09-10 起）** | ✅ **（2026-09-10 起）** | NS → AP → RT → ENEX → ABC |
...
跟本檔 V5-1 一樣：狀態檔目前的 `checkpoint` → 現在（`resume` 看，不要猜），
...
⭐ **三站（NS／AP／RT）正常批次路徑（V9 權威版，2026-09-14 消除舊文互斥）**：

1. `browser_evaluate` 抽完（§0-1 白名單）落 raw 檔。
2. `python scripts/s2_batch_prep.py from-raw --site <ns|ap|rt> --raw <raw> --checkpoint <MMDD-HHMM> --state <狀態檔>`
   → 提示表＋骨架 json（已 `has_script` 的排除；pending／prelim／early 保留並標 `prev_status`）。
3. 看提示表寫 entries.json：`{id:{entry,category,tc}}`，要開新中主題放頂層 `_new_topics`。
4. `python scripts/s2_batch_prep.py build --site <站> --raw <raw> --checkpoint <MMDD-HHMM> --skeleton <骨架> --entries <entries.json> --out <batch.json>`
   （`--site/--raw/--checkpoint` 必填，漏了 CLI 當場退回）→ 出 `{entries,new_topics}` batch；
   `build` 會印「ID: 格式原因」警告——**在 entries.json 草稿裡改掉再 build**（只警告、不擋、不替你改）。
   送出前照舊：①b 自查（`13c2` §2「①b 稽核前置」）；NS 必跑 `compare --require src_text`（`13c2` §5）。
5. 一站一次 `add-batch`：分類／T/C 隨批次入庫。`set-category`／`set-tc` **只用於事後修補**，不是每站固定再跑一次。

- 撤回史（不必照做）：0812 強制「`dump`→手轉 schema→`build`」三步流程逼出大量臨時 python（呼叫 90→203），已撤回；
  現行 `from-raw` 直接吃 raw 檔，不需要手寫轉檔，兩者不是同一件事。`dump` 仍是選用。
- `inspect`／`unwrap`／`search` 是檢查暫存 raw 檔用，見 `13c2` §4；`inspect` 只查提示表看不清的疑點（T9 預算制照舊）。
...
2. **只讀要用的欄位，⛔ 不要整包 `Read` 進 context**：卸載檔是**沒瘦身的原始回應**，
   整包讀進來等於把 §0-1 省下的 6–9 成又賠回去。卸載檔先
   `s2_batch_prep.py unwrap <卸載檔> --out <raw.json>` 卸殼成裸陣列（`from-raw` 不認工具外殼），
   再當 `from-raw --raw` 的輸入，接回 §1a 正常批次路徑。
   （`browser_evaluate filename=` 直接落地的檔本來就是裸陣列，不必 unwrap。）
3. **⛔ 不要另外寫合併腳本／手寫 batch.json**：交給 `from-raw`→`build --skeleton`。
   **臨時腳本是這條路上最貴的一段**，而且每輪重寫一次、每次都可能寫錯（0811 那支就改了兩次）。
...
- ⛔ **嚴禁無頭（headless）跑 RT**——一律用有畫面（headed）瀏覽器。
...
- 🔴 **`browser_navigate` 一律開 AP 官方「All Latest」topic 頁，⛔ 不是 `/home` 首頁**
  （2026-08-20 訂正；`/home` 的 Latest 小工具清單範圍與排序都不同，是「每輪只收 16-17 則」的根因）：
  **`https://newsroom.ap.org/topic?id=116e9ab7aa044476925398d731289267&mediaType=video&navsource=latest&parentlnk=false`**
...
- ⭐ **AP 清單查詢：照抄即用範本（0821 新版，fetch＋瘦身一次做完，取代舊版）**（`browser_evaluate` 一次呼叫；
  **不要分好幾次呼叫、不要自己刪減欄位、⛔ 不要回傳 `r.json()` 原始整包**）：
    ...
    - 🔴 **Page1（50 則）＝唯一可信來源，全部收錄。`PageNumber:2` 不接續 Page1**——不論
      `PageSize` 多少都固定回 100 則、對齊伺服器內部 offset，會出現空窗或重疊。
      ⛔ **Page2 只能當候選補充池**（去重＋逐一核實或 needs-review），不准直接當確定則寫入。
      詳見 MASTER R6。
...
- ⭐ **抽取白名單**＝上方兩支範本內嵌的那套，⛔ 不要自己另寫一份。兩個藏在裡面的規則：
  `sb_count` 的 `/SOUNDBITE/g` **不可加 `i` 旗標**（理由同 RT）；`prelim` 只認**開頭**的
  `++PRELIMINARY SCRIPT`，內文提到不算。
...
() => {
  ...
  // 2026-09-07 V8 合入本檔；配方以此為準，⛔ 禁翻 investigation-logs
  const body = {
...
- 🔴 **AP `PageSize` 偏離範本值（現行 `50`，見 §2 AP）── 本身就算一次失敗，不是「換個參數再試試」**
