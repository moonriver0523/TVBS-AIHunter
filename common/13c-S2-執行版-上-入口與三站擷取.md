...
| 檢查點 | 掃 ENEX？ | 依據 |
|---|---|---|
| 01:00 | ⬜ **跳過（2026-09-09 起）** | 使用者下令收掉，理由：01:00＋04:30 各掃一次 ENEX 的固定開銷（extract/merge/lint 全套流程約 8-10 次呼叫）跑兩輪，改回只在 04:30 掃一次可省一輪固定開銷；素材延後到 04:30 收，不會漏收。~~2026-09-05 起 ✅：原本只補三站不掛 ENEX；使用者明確下令改掛，理由同 04:30／07:00：發布延遲會落進這個窗~~ |
...
|---|---|---|---|
| 01:00 | ⬜ **跳過（2026-09-09 起）** | ⬜ **跳過（2026-09-09 起）** | NS → AP → RT（同 11:00／20:00 的跳過寫法：不查、不寫候選檔，回報寫一行「本輪不掃 ENEX／ABC」）~~2026-09-05 起 ✅✅：NS → AP → RT → ENEX → ABC~~ |
