...
| 檢查點 | 掃 ENEX？ | 依據 |
|---|---|---|
| 01:00 | ⬜ **跳過（2026-09-09 起）** | 使用者下令收掉，理由：01:00＋04:30 各掃一次 ENEX 的固定開銷（extract/merge/lint 全套流程約 8-10 次呼叫）跑兩輪，改回只在 04:30 掃一次可省一輪固定開銷；素材延後到 04:30 收，不會漏收。~~2026-09-05 起 ✅：原本只補三站不掛 ENEX；使用者明確下令改掛，理由同 04:30／07:00：發布延遲會落進這個窗~~ ⚠️ ENEX 本身沒有 ABC 那種 session 保活問題（ENEX 認證機制不同），這條「跳過」單純是省全量擷取，跟 V7-5 摸 ABC cmspage 續期是兩件事，見下方 V7-1 表 01:00 列的訂正 |
| 04:30 | ✅ | 涵蓋 **22:00–04:30（2026-09-09 起，因 01:00 不再掃回到舊窗）**，31 天 597 則（每輪約 19 則，全日最多）；⚠️ 併回舊窗後預期單輪耗時增加，觀察期樣本不足前先每輪覆核實際耗時，見 MASTER R33 附近討論 |
...
|---|---|---|---|
| 01:00 | ⬜ **跳過（2026-09-09 起）** | ⬜ **跳過（2026-09-09 起）** | NS → AP → RT（同 11:00／20:00 的跳過寫法：不查、不寫候選檔，回報寫一行「本輪不掃 ENEX／ABC」）~~2026-09-05 起 ✅✅：NS → AP → RT → ENEX → ABC~~ ⚠️ **「跳過」只免掉全量清單/詳情擷取，不免 `13c1b` V7-5 收工摸一次 ABC cmspage——那條規則跟本表無關、每輪都要做，不准因為本輪跳過 ENEX/ABC 而連摸都不摸（保活腳本在掃帶跑的時候撞鎖會整支 SKIP，不會補上）** |
