...
- （2026-09-07 A24）batch 由 `s2_batch_prep.py build --skeleton <骨架> --entries <entries.json>` 產，agent 只寫 `{id:{entry,category,tc}}`；要開新中主題就在 entries.json 頂層放 `_new_topics:{名:{charter,big}}`（沒新題可以不放）。`build` 一律出 `{entries,new_topics}` 外殼。
