# 13c2　S2 定時掃帶 V9（執行版）— 下：狀態檔與指令

> **V9（2026-09-14）**：以 V8 五層合一為底；只改批次路徑／分類 T/C 互斥句（§2、§4）並補 `rename-field`。V8 原檔在 `common/v9/_v8bak/`，`s2_v9_switch.ps1 -Off` 切回。
> 分類／T/C 正常隨 `add-batch` 入庫；§2 的 `set-category` 只剩「事後修補」用途。
...
3. 🔴 **清單是內層容器的虛擬捲動，首屏只渲染約 10 則**——不捲只看得到 1/6（0805 實錯：agent 只「找到」2 則、實際窗內 36 則未收）。`window.scrollBy` 無效，要捲 `.homepage` 那個 `overflow-y:auto` 的內層 div，**邊捲邊累積**（捲過去的會從 DOM 消失）：
   ...
   ```js
   await new Promise(r => setTimeout(r, 1500));   // 等篩選／排序套完再撈（R27）
   ...
   const grab = () => document.querySelectorAll('a[href*="/detail?id="]').forEach(a => {
     ...
     const dm = t.match(/(\d{2}\/\d{2}\/\d{4}\s\d{2}:\d{2})/);   // 上站時間，⛔ 不要加 ^（卡片開頭是 EARLY ACCESS／時長，日期不在句首）
   ...
   return { noAt: out.filter(x => !x.at).length, items: out };   // 殼鍵 items，s2_batch_prep 認得
   ```

   一次約 60 則、涵蓋前 5 個多小時，正常輪次夠用。
   🔴 **撈完先看 `noAt`（R27，0907 兩輪實錯）**：`noAt` ≥ 一半或 `items` < 3 則＝頁面還沒套完篩選，**這份清單不能用**——等 3 秒再撈一次；第二次仍空就 `needs-review add` 記「RT 清單無上站時間」。⛔ 沒有 `at` 的清單不准拿來判窗內外、不准當對帳快照（0907-1700／2000 首抓 `at` 全空且少 9 則，靠 `s2_audit` Ⓐ 才補回）。
...
python scripts/s2_state.py add-batch --entries batch.json  # ⭐ 批次新增（預設路徑）
                                ...
                                #    `category` 收字串「大/中/小」**或**狀態檔同形物件（R25）；
                                #    `tc` 收「T1,T2/C1」或 {"T":[…],"C":[…]}。
...
- 🔴 **⛔ 不准自己寫 python 去讀／改狀態檔**（2026-08-11 立規，實測最大宗的浪費）：
  ...
  - **成本主因是呼叫次數**（每次呼叫都要重讀整個 context），不是規則檔多長。
    `show`／`--batch` 已補掉舊工具箱缺口，正規指令永遠比臨時腳本省。
  - ⚠️ 臨時腳本特別容易自傷：Windows 沒有 `/tmp`；主控台是 **cp950**，印中文／emoji
    直接 `UnicodeEncodeError` 整段白跑。⛔ **這裡不給防護樣板**——給了樣板等於發許可證
    （0812 放樣板當天臨時 python 從 30 次衝到 54 次）。**這條規則是「不要寫」，
    不是「怎麼寫得安全」。** 正規腳本開頭已內建防護。
  - 📌 真的遇到 `show` 查不到的欄位：**回報說缺什麼**，不要繞路自己寫。
  - 📌 工作 batch 鍵名寫錯（如 `raw_entry`→`entry`）：`python scripts/s2_batch_prep.py rename-field <batch.json> --from 舊 --to 新`（出新檔、不原地改、不碰狀態檔），⛔ 不要 `python -c` 改字串。
- 🔴 **⛔ 寫入子指令要批次、不准一則一次**（2026-08-12 立規；V9 口徑）：
  `add-batch` **一站一次**；`update-entry --batch fix.json` 本站收尾一次；`set-category` 只在修補時用（§2）。
  把該批要改的全部塞進同一次呼叫——`set-category --pairs "a=…;b=…;c=…"`。⛔ **不准一則一次、不准跑迴圈**。
  - **做法**：邊分類邊把 `id=大/中/小` 累積成一份清單，**全部看完才送出**。
    中途發現前面分錯了就改清單裡那一行，不要另外再叫一次補正。
    （每次工具呼叫 ≈ 0.25M cache_read，多叫 12 次約白燒單輪的 10%。
    ⚠️ 自己算 token 要先依 `message.id` 去重，否則灌水約 1.8 倍。）
...
- （2026-09-07 A24）batch 由 `s2_batch_prep.py build --site --raw --checkpoint --skeleton <骨架> --entries <entries.json>` 產（前三個參數必填），agent 只寫 `{id:{entry,category,tc}}`；要開新中主題就在 entries.json 頂層放 `_new_topics:{名:{charter,big}}`（沒新題可以不放）。`build` 一律出 `{entries,new_topics}` 外殼。
- **批次擷取流程**（三站權威順序見 `13c` §1a「正常批次路徑」）＝收本輪列表 ID → 取文稿落 raw → `from-raw --state` 骨架 → 寫 entries（含 category／tc）→ `build --skeleton` → **一站一次 `add-batch`**。⛔ 不要一則一次 `add`（呼叫次數是變慢主因：25 則從 52 次降到 4 次）。
- **批次規則**：🔴 RT／NS／AP 的 `batch.json` **一律是 `{"entries":[…],"new_topics":{…}}`**（2026-09-08 P1b-2 硬上線；純陣列 `add-batch` 當場退回，退回訊息就是修法。ENEX／ABC 平台線不受限）。每筆 `{"id","source","checkpoint","status","entry"}`。撞 id 或格式錯的單筆自動跳過並回報（已存在→改 `update-entry`；格式錯→修後單筆補）。`--pairs` 分隔符用分號 `;`。
...
- **整併流程**＝pending 全量重查 → `add-batch`（帶 category／tc）／`update-entry` →（僅需修補時）`set-category --pairs` → 側錄 `add-side` → `set-alert` → `s2_render.py` 全量渲染。agent 輸出趨近 0，不手寫 txt。
...
## 2. 分類／T/C 隨 `add-batch` 入庫；`set-category` 只做事後修補（V9）

- **正常路徑**：分類與 T/C 寫在 entries.json 的 `category`／`tc`，隨 `build --skeleton` → `add-batch` 一次入庫（T12）。
  ⛔ 已隨批次正確入庫的站，**不要再固定下一次 `set-category`／`set-tc`**（V8 舊文「各站掃完各下一次」已取代）。
- **修補才下**：批次沒帶到、被退件（`tc_rejected`）、或事後發現分錯時，一站最多一次 `set-category --pairs`，照舊累積清單一次送出，⛔ 不准一則一次。
- **收尾允許 ≤1 次補刀**：三站都跑完、`topic_review` 看過整張表、發現同義／同事件需要合併分類，**最多再補一次** `set-category --pairs`。
  超過就是踩線——真的還有錯，累積到下一輪或用 `needs-review add` 記錄。
- 順序仍是**先累積草稿、看過沒問題才送出**（本檔 §2「①b 稽核前置」不變）。
...
⛔ **不要即興檢查暫存 raw 檔**（`Read` 整檔讀進 context／PowerShell 切片／`python -c`
一次性腳本，三種都算違規），理由同 13c §2「不准自己寫 python 讀狀態檔」。
...
- 用途是**檢查**（結構、筆數、有沒有拆對層）。三站主路徑是 `13c` §1a「正常批次路徑」
  （`from-raw` → `build --skeleton` → `add-batch`），不是 0812 撤回的「手轉 schema＋`dump`」流程。
...
`add-batch` 的每一筆都要帶 `src_text`（抽取白名單瘦身後的站方原文）——
