...
- （2026-09-07 A24）batch 由 `s2_batch_prep.py build --skeleton <骨架> --entries <entries.json>` 產，agent 只寫 `{id:{entry,category,tc}}`；🔴 entries.json 頂層**一律放 `_new_topics`**（沒新題就 `{}`；要開新名就 `{名:{charter,big}}`）——有這個鍵產物才是 `{entries,new_topics}` 新格式、才過 P1b 閘門（沒放＝純陣列、不過閘，登記簿會脫鉤）。
