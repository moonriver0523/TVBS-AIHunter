...
| ID | 項目 | 來源 | 狀態 | 要看什麼／驗收 |
|----|------|------|------|----------------|
| E1 | RT Load More 迴圈判定：頁面行為改變 vs 策略漂移 | 交接§6-1 | ⏸ | 2200 輪 RT 呼叫模式（捲動探測次數、Load More 點擊、清單重抽）。仍迴圈→啟動 R1；消失→再觀察 0030 輪才結案。⚠️ 2200 同輪吃到 effort medium 變更，歸因要看呼叫序列不能只看總量 |
| E2 | effort=medium 實效驗證 | 093c713 | ⏸ | P2 遙測雙口徑：耗時＋output/cache token，對照 0811-2200（high、117次/29.5分）。數據交 D1 裁決 |
| E3 | 鎖檔收工自動清理實戰驗證 | c9614ac | ⏸ | 2200 輪 DONE 後 `%USERPROFILE%\.s2-scan.lock` 應不存在 |
| E4 | ①b 稽核前置驗收：update-entry 次數 7→1~3 | TODO | ⏸ | 用 P2 遙測的 `s2_state:update-entry` 計數看 2200 輪（1200 輪曾為最省輪 90 次，未單獨驗過此項） |
