...
|----|------|------|------|------------|
| E1 | RT Load More 迴圈判定：頁面行為改變 vs 策略漂移 | 交接§6-1 | ⏸半結 | 連兩輪無迴圈，判定為策略漂移偶發，結案；R1 是否仍要預防性寫死交使用者裁決 |
...
| E6 | 輪次局部效率下降非則數所致，疑似偶發性行為/效率漂移 | 0819-2200 體檢＋使用者覆核 | ⬜列入觀察 | 首次樣本：0819-2200耗時41.5分為當日最長，但三站則數其實沒增加(RT反而最少)；真正異常是RT呼叫數/單次分類呼叫耗時翻倍，具體表現為RT/NS對同一則反覆小幅裁切字數(重複做而非資料量大)；與D9/E5同屬偶發行為漂移家族，累積樣本再議 |
