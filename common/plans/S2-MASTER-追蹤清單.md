...
| A10 | S2 分類系統化（大／中／小三層） | 分類規劃（2026-08-18） | — | 🗳待裁決啟動 | 三道閘未過，P0 未結案前只准做 P0。**2026-08-21 補**：使用者提案 T/C 標籤矩陣（議題×地緣雙軸）併入本案，已寫入規劃書 §6b（D-h~D-j 三題併入 §3b 裁決包），未授權實作 |
...
| A21 | 🔴 (BITE)標籤因無自動判斷工具標錯，事後整批重寫 | 0821-2000體檢(RT，2則)；0822-0100體檢大幅升級(AP，29/30則幾乎整批)＋NS標籤位置錯 | — | ⬜列入觀察，規模已升級建議提高優先度 | RT首次樣本僅2則標反；**0822-0100樣本暴增**：`ap_fix_bite_0100.json`重寫`ap_batch_0100.json`30則中的29則(幾乎整批)，另NS `ns_fix_summary_0100.json`2則出現「(BITE)」標籤位置跑到時長後面而非標題區的變體錯誤。三站(RT/AP/NS)都已出現樣本，且AP這次是近乎全批規模，不再是零星個案，建議列為A2缺口裡優先度較高的一項 |
