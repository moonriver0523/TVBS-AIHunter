...
| A18 | AP清單漏套抽取白名單(原始ES回應整包落檔)＋AP詳情第一次擷取又壞掉 | 0821-0430體檢(使用者要求深查，main查證) | — | ⬜列入觀察 | 問題一：`ap_list_0430.json` 927KB/50則(約18.5KB/則)，比同類其他輪(0.5KB/則)大近40倍，確認是未過濾原始ES回應，完全沒套13c§0-1強制白名單瘦身規則——首次抓到具體可量化的真實違規樣本(先前都只有理論估算)。問題二：`ap_detail_0430.json`第一次擷取id變成`APAP5467923`雙重前綴、欄位全空，重試後`ap_detail_0430b.json`才正常——與0820-2200輪`ap_list_2200.json`全部`APundefined`同一種故障簽章第二次出現，懷疑AP擷取JS有間歇性bug非單次偶發。兩者均僅1-2樣本，列入觀察 |
