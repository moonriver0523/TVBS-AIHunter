...
| D9 | `python -c` 硬排除（A2 行為漂移的硬解） | A2 0817 觀察 | 🗳 | 0817 五輪 `python -c:json` 呈 26／33／0／0／36——13c §1a 的文字澄清擋不住，形狀跟 T2「Task 工具靠自律會漂移」完全一樣，而 T2 的解是 launcher `--disallowedTools` 硬排除。可行做法：把 `Bash` 裡的 `python -c` 走**不排除但改導引**（風險低）或比照 T2 直接擋掉（風險高——`python -c` 有正當用途，例如查狀態檔以外的一次性計算，全擋可能逼出更糟的變通）。**兩條路都會改變 agent 行為，未裁決前不動工**。0817-2000 逐筆查證後，建議收斂成三段式而非一刀全擋：①**檢查 raw／batch**（8/14 筆）→ 已有 `inspect`／`compare`，可硬擋；②**讀生產狀態檔**（3/14 筆）→ 13c §2 早就明文禁止，硬擋無爭議；③**數中文字數**（3/14 筆）→ 沒有替代工具，硬擋會逼出更糟的變通，應先補一個 `s2_state.py`／`s2_batch_prep.py` 的字數檢查子指令再談。⚠️ 若要走工具層硬擋，比對字串必須同時涵蓋 `python -c` 與 **`python3 -c`** **0817-2200 補證**：新做的 `snapshot` 子指令當輪就被 agent 用 ad-hoc python 重造了一次（自寫 `_audit_ap_2200.json`）——顯示「補工具」單獨做不夠，必須配合工具層導引／硬擋，否則新工具只會被忽略。 |
