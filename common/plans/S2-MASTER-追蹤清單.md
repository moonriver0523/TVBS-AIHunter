...
| A10 | **S2 分類系統化**（大／中／小三層精準度＋跨輪一致性，TODO「全盤檢討分類問題」結案路徑） | 分類規劃（2026-08-18） | — | 🗳待裁決啟動 | 規格一律看分類規劃文件，本表不重抄。結構：P0 歷史交接語料分析→一次性裁決包（層級模型／社會與話題定位／中主題粒度命名／小分題機制擇一）→P1 主題登記簿 charter/aliases＋分類判例庫→P2 收斂輪制度化。⚠️ 三道閘：①「檢討完成前不做分類機械檢查／自動修正」禁令仍生效，P0 沒結案只准做 P0；②P0 歷史分析啟動前要先向使用者報價確認（高 token）；③裁決包 7 題（D-a~D-g）屬 D-類，未裁決不動工。前置：歷史範例前 10 份 `.gdoc` 本機讀不到，需請使用者轉檔或走 Drive |
