...
| A19 | AP詳情擷取無謂拆成3次呼叫(a/b/c各10/10/8則) | 0821-1600體檢(使用者要求查AP疑似浪費) | — | ⬜列入觀察 | `ap_detail_1600_a/b/c.json`共28則分3次`browser_evaluate`各自呼叫，而13c§1a新範本已示範單次`Promise.all`可處理20則以上(main剛Playwright模擬驗證過)，理論上28則本可1次做完。與A17(RT batch無謂拆成6段Write)同屬「無謂拆分」浪費家族，但這次出現在AP的**擷取**階段而非**寫入**階段，機制不同、分開追蹤。僅1個樣本，先列入觀察，累積更多輪確認是否規律復發 |
...
| A21 | RT的(BITE)標籤因無自動判斷工具標反，整批重寫一次 | 0821-2000體檢(使用者要求查2000輪疑似浪費) | — | ⬜列入觀察 | `rt_fix_2000.json`→`rt_fix2_2000.json`：RT6991(稿未到、實際無BITE)第一次誤標「BITE：（稿未到）」、RT6906(實際有BITE)第一次標題漏掛「(BITE)」，兩則標籤都標反，第二次才全部改對。與A2/A20同屬「無自動判斷工具、靠人工判斷易錯」的缺口家族，但這次是BITE有無判斷（非字數），且出現在RT站。僅1個樣本，先列入觀察 |
