...
| D9 | `python -c` 硬排除（A2 行為漂移的硬解） | A2 0817 觀察 | 🗳 | 0817 五輪 `python -c:json` 呈 26／33／0／0／36——13c §1a 的文字澄清擋不住，形狀跟 T2「Task 工具靠自律會漂移」完全一樣，而 T2 的解是 launcher `--disallowedTools` 硬排除。可行做法：把 `Bash` 裡的 `python -c` 走**不排除但改導引**（風險低）或比照 T2 直接擋掉（風險高——`python -c` 有正當用途，例如查狀態檔以外的一次性計算，全擋可能逼出更糟的變通）。**兩條路都會改變 agent 行為，未裁決前不動工** |
