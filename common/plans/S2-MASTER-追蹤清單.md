...
- [ ] **（2026-08-18 0430/0730 體檢新發現，未修）「分類」桶把 NS/AP/RT 組稿寫入的長思考也吞了**：0813-0430 那次修的是 Task 工具關鍵詞誤判，這次是**不同觸發源、同一種現象**——`classify_phase` 判斷順序是「先比對關鍵字，比不到就繼承前一階段」，而 `list-topics --compact`／`show --mid <中主題>`（agent 查現有分類寫法以求措辞一致，是正確習慣）本身比對到「分類」關鍵字，但**緊接著的 `Write batch_ns/ap/rt_*.json`（組稿本體）與 `add-batch` 指令內容都不含 NS/AP/RT 關鍵字**（`ns_batch`／`batch_ns` 這種檔名不含 `_ns_`），於是整段組稿的長思考被黏著繼承成「分類」。**逐筆核對量化**：0818-0430 報 分類=15.0 分，其中 NS 組稿前思考 425s＋AP 組稿前 94s＋RT 組稿前 213s＝732s（12.2 分）其實是三站寫稿，真分類 ≈2.8 分；0818-0730 報 分類=9.4 分，NS 250s＋AP 116s＋RT 75s＝441s（7.4 分）同樣是寫稿，真分類 ≈2.0 分。**不影響掃帶行為，只是報表誤讀**——下個 agent 拿「分類 X 分」當異常訊號前，先查是否跟在 list-topics/show 後面的 Write 大量思考。修法方向：`classify_phase` 對站別關鍵字加 batch 檔名變體（`ns_batch`／`batch_ns` 兩種順序都要收），或乾脆讓 `Write`／`add-batch` 一律按檔名或 `--entries` 路徑裡的站別字樣重新判定，不吃繼承
