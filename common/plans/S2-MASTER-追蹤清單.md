...
| R6 | AP 對帳清單抓到 page 2（≥32 則） | 接手查證 | 6 | ✅2026-08-13（13d §7） | 現況 list=16 恰為 page 1 容量，被擠出 page 1 的素材對帳網接不到（AP5467302 實例）。掃描本身有翻頁（三輪 log 均有 PageNumber=2），弱的是對帳快照。修法走規則層：13d §7 要求對帳快照 page1+page2 合併（≥32 則，當窗不足 17 則才允許單頁）。屬規則文字，待下輪實戰驗收快照筆數。**0814-0100 實戰：agent 改走 API 一次撈 117 則（超額達標），但落地格式變 tab＋ISO UTC，audit parser 只認 `\|` 分隔→整行當 code、時間空白→117 則全進「窗外」假綠燈（已收 0 竟無紅燈）。main 當場修 parse_list_file（tab 分隔／total 表頭跳過／ISO UTC→本地時區），重跑：AP 已收 58＋前幾天 33＋**窗內未收 23**——全窗對帳首戰就抓到被 page1 快照時代遮住的累積漏收，23 則清單已入稽核紀錄，待 0430 輪核實回補** |
